# A1 · Lektion 14 — Prüfungstraining A1 (Mock Exam)

---

## Übung exam-H1 — Aufgabe 1 — Kurze Gespräche

1. **a) Links (nach geradeaus und rechts abbiegen)** — Geradeaus gehen, dann rechts abbiegen — der Bahnhof ist dann links.
2. **b) Orangensaft und ein Käsebrötchen** — Die Frau sagt: "Ich möchte einen Orangensaft und ein Käsebrötchen."
3. **b) Um 8:00 Uhr** — Lena steht um halb sieben auf, aber der Kurs beginnt um acht.
4. **b) Sie geht nach Hause und schläft.** — Selin sagt explizit "Nein" auf die Frage nach dem Arzt.
5. **c) Beim Arzt** — Wartezimmer + Doktor Berger = Arztpraxis.

## Übung exam-H2 — Aufgabe 2 — Richtig oder falsch?

1. **F** — Das Konzert beginnt um 20 Uhr, nicht um 19 Uhr.
2. **R** — Petra sagt: "ich habe die Karten jetzt."
3. **R** — Viertel vor acht = 19:45 Uhr. Am Bahnhof.
4. **F** — Petra kaufte ein Ticket für Marias Schwester, nicht den Bruder.
5. **R** — Petra sagt: "meine Nummer ist 0173 456 789."

## Übung exam-H3 — Aufgabe 3 — Kurze Ansagen

1. **b) Gleis 7** — Der Zug fährt von Gleis 7 ab — nicht Gleis 5 (das alte Gleis).
2. **b) Von 9 bis 18 Uhr** — Samstags von 9 bis 18 Uhr (Mo–Fr 9–20, So geschlossen).
3. **b) 0,99 €** — Neunundneunzig Cent = 0,99 €.
4. **b) Die Lehrerin ist krank.** — Frau Hoffmann ist krank — deshalb kein Kurs heute.
5. **c) 6 Euro** — Eintritt: sechs Euro. Kinder unter 12 Jahren zahlen nichts.

## Übung exam-L4 — Aufgabe 4 — Anzeigen und Aushänge

1 → E · 2 → A · 3 → C · 4 → B · 5 → D

## Übung exam-L5 — Aufgabe 5 — Kurze Texte: Carlos Fernández

1. **b) Aus Mexiko** — Carlos kommt aus Mexiko.
2. **c) Koch** — Carlos arbeitet als Koch in einem Restaurant.
3. **a) Am Montag und Dienstag** — Am Montag und Dienstag hat Carlos frei.
4. **b) Montags um 9 Uhr** — Sein Kurs ist montags um 9 Uhr.
5. **c) In Mexiko** — Carlos' Frau und Kinder sind noch in Mexiko.

## Übung exam-L6 — Aufgabe 6 — Formular lesen: Yuki Tanaka

1. **F** — Yuki kommt aus Japan (Geburtsort: Osaka, Japan).
2. **R** — Wohnort: Berlin. ✓
3. **R** — Geburtsdatum: 14. März 1995. ✓
4. **F** — Der Kurs beginnt am 5. September — nicht im Oktober.
5. **R** — E-Mail: yuki.tanaka@mail.de. ✓

## Übung exam-L7 — Aufgabe 7 — Nachrichten lesen

1 → Selin · 2 → Lisa · 3 → Marco

## Übung exam-S8 — Aufgabe 8 — Formular ausfüllen

Modellantwort:

> Vorname: Anna
> Nachname: Berger
> Geburtsdatum: 12.04.1990
> Straße und Hausnummer: Hauptstraße 5
> Postleitzahl und Wohnort: 20095 Hamburg
> Telefonnummer: 0176 12 34 56 78
> E-Mail: anna.berger@mail.de
> Unterschrift: Anna Berger

Selbstkorrektur:
- [ ] Alle 8 Felder ausgefüllt? (0,75 Pkt. × 8 = 6 Pkt.)
- [ ] Datumsformat korrekt (z.B. 12.04.1990)?
- [ ] PLZ = 5 Ziffern?
- [ ] E-Mail-Format korrekt (name@domain.de)?

## Übung exam-S9 — Aufgabe 9 — Kurze Mitteilung schreiben

Modellantwort:

> Liebe Anna,
> ich kann leider am Dienstag nicht in den Kurs kommen — ich habe einen Arzttermin.
> Ich komme wieder am nächsten Dienstag. Kannst du mir bitte die Hausaufgaben schicken?
> Viele Grüße,
> [Name]

Selbstkorrektur:
- [ ] Gruß am Anfang und Ende vorhanden? (0,75 Pkt.)
- [ ] Grund für Abwesenheit angegeben? (0,75 Pkt.)
- [ ] Rückkehrdatum erwähnt? (0,75 Pkt.)
- [ ] Um Hausaufgaben gebeten? (0,75 Pkt.)
- [ ] Verständliches Deutsch / Verb in Position 2? (1,5 Pkt.)
- [ ] Ca. 30–40 Wörter? (0,75 Pkt.)

## Übung exam-Sp10 — Aufgabe 10 — Sich vorstellen

Bewertungskriterien:
- Alle 6 Impulse mit grammatisch akzeptablen Sätzen beantwortet — 2 Pkt.
- Aussprache und Verständlichkeit — 1 Pkt.
- Folgefragen (W-Fragen) flüssig beantwortet — 1 Pkt.
- Interaktion: Partner begrüßt, Blickkontakt — 1 Pkt.

## Übung exam-Sp11 — Aufgabe 11 — Alltagssituationen

Bewertungskriterien:
- Kommunikative Aufgabe erfüllt (Gesprächspartner reagiert korrekt) — 2 Pkt.
- Grammatisch akzeptabel (Verben konjugiert, Artikel weitgehend korrekt) — 2 Pkt.
- Interaktionsqualität (Sprecherwechsel, Höflichkeit) — 1 Pkt.

## Übung exam-B12 — Aufgabe 12 — Lückentext: Grammatik gemischt

1. **bin**  2. **haben**  3. **zum**  4. **hat**  5. **gekauft**  6. **Welches**  7. **mit**  8. **bitte**  9. **mir**  10. **stehst**  11. **bin**  12. **gewesen**

## Übung exam-B13 — Aufgabe 13 — Finde den Fehler

Selbstkorrektur:
- [ ] 1. habe → bin (gehen = Perfekt mit sein)
- [ ] 2. Sie → (entfernen) — kein Sie nach du-Form: Kannst du morgen kommen?
- [ ] 3. aufstehen morgen → morgen früh aufstehen (Zeitangabe vor Partikel)
- [ ] 4. keinen → keine (Kinder = Plural → keine)
- [ ] 5. ich → mir (gefallen + Dativ: Er gefällt mir)
- [ ] 6. ein → einen (der Termin, mask. Akkusativ → einen)
- [ ] 7. letztes → letzte (die Woche, fem. → letzte)
- [ ] 8. sportliche → sportlich (prädikatives Adjektiv nach sein: keine Endung)

## Übung exam-B14 — Aufgabe 14 — Übersetzen (EN → DE)

Selbstkorrektur:
- [ ] 1. Wann fährt der Zug ab? (Trennverb abfahren; Verb in Pos. 2)
- [ ] 2. Ich möchte ein Glas Wasser, bitte.
- [ ] 3. Sie kann heute nicht kommen — sie hat Kopfschmerzen.
- [ ] 4. Wir haben gestern einen neuen Tisch gekauft. (neuen = Akk. mask.)
- [ ] 5. Haben Sie einen Arzttermin? (einen = Akk. mask.)
- [ ] 6. Er arbeitet als Koch in einem Restaurant. (als + Nom.; in einem = Dat.)

## Übung exam-B15 — Aufgabe 15 — Satzbau

1. Ich stehe um sieben Uhr auf
   _Trennverb auf·stehen: auf an letzter Stelle_
2. Können Sie bitte Ihren Namen buchstabieren ?
   (auch richtig: Können Sie Ihren Namen bitte buchstabieren ?)
   _Modalverbsatz: Infinitiv am Ende; bitte auch nach Namen möglich_
3. Meine Mutter hat gestern Obst gekauft
   _Perfekt: hat … gekauft_
4. Der Zug ist um 14:32 Uhr abgefahren
   _Perfekt mit sein; abgefahren am Ende_
5. Ich freue mich sehr auf den Sommer
   _Reflexiv: freue mich; Akkusativ: auf den Sommer_

---

## Bewertung

| Prüfungsteil | Punkte | Schwelle |
|---|---|---|
| Teil 1 · Hören | / 15 | ≥ 8 |
| Teil 2 · Lesen | / 18 | ≥ 10 |
| Teil 3 · Schreiben | / 12 _(Selbstbewertung)_ | ≥ 6 |
| Teil 4 · Sprechen | / 10 _(Selbstbewertung)_ | ≥ 5 |
| **Gesamt** | **/ 55** | **≥ 29** |

_Gesamt ≥ 29/55 UND Teilschwelle in mindestens 3 von 4 Prüfungsteilen erreicht_
