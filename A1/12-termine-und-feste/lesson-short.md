---
level: A1
number: 12
slug: termine-und-feste
title: Termine und Feste
titleEn: Appointments and Celebrations
canDo:
  - say and write dates in German
  - talk about months, seasons, and public holidays
  - make, accept, and decline appointments
  - write a short birthday invitation
  - wish someone a happy birthday or a good holiday
grammar:
  - ordinal numbers and dates (am ersten, am zweiten…)
  - temporal prepositions im, am, um, vor, nach
  - making and declining appointments (sentence patterns)
  - separable verb einladen (recap)
buildsOn:
  - A1/01 (numbers 0–20)
  - A1/05 (time expressions um, am; separable verbs)
  - A1/06–07 (modal verbs können, müssen, wollen)
  - A1/11 (Perfekt — eingeladen)
---

# A1 · Lektion 12 — Termine und Feste (Appointments and Celebrations)

> **You will learn to:** say and write dates, talk about months and holidays,  
> make and decline appointments, write a short invitation, and wish someone a
> happy birthday.
>
> **Grammar:** ordinal numbers and dates (`am ersten, am zweiten…`) ·  
> temporal prepositions `im`, `am`, `um`, `vor`, `nach` ·
> making and declining appointments.
>
> **Builds on:** numbers (L1, L2), separable verbs (L5), time expressions `um`  
> and `am` (L5), modal verbs `können`, `müssen`, `wollen` (L6, L7), Perfekt
> (L11).

---

## 1. Dialog: Haben wir heute einen Termin?

🎧 **Audio:** [dialog1_a.mp3](audio/dialog1_a.mp3)

### Dialog A: Informell — Anna und Bruno planen eine Party

> **Anna:** He, Bruno! Mein Geburtstag ist am dreizehnten Mai. Ich mache eine Party.  
> **Bruno:** Super! Wann denn?  
> **Anna:** Am Samstag, dem vierzehnten Mai, um sieben Uhr abends.  
> **Bruno:** Oh, Moment. Den vierzehnten — das geht leider nicht. Ich habe einen Termin.  
> **Anna:** Und am Fünfzehnten, am Sonntag?  
> **Bruno:** Ja, das passt! Ich komme gern.  
> **Anna:** Prima! Ich schicke dir eine Einladung.  
> **Bruno:** Vielen Dank — und herzlichen Glückwunsch zum Geburtstag!  
> **Anna:** Ha, der ist erst am Dreizehnten! Aber danke!  

🎧 **Audio:** [dialog1_b.mp3](audio/dialog1_b.mp3)

### Dialog B: Formell — Frau Weber macht einen Arzttermin

> **Sprechstundenhilfe:** Guten Tag, Praxis Dr. Hoffmann. Was kann ich für Sie tun?  
> **Frau Weber:** Guten Tag. Ich möchte gern einen Termin beim Doktor.  
> **Sprechstundenhilfe:** Gern. Passt Ihnen Mittwoch, der achtzehnte Oktober?  
> **Frau Weber:** Um wie viel Uhr?  
> **Sprechstundenhilfe:** Um 10 Uhr 30.  
> **Frau Weber:** Ja, das passt gut. Ich komme am Mittwoch um halb elf.  
> **Sprechstundenhilfe:** Sehr gut. Auf Wiederhören, Frau Weber!  
> **Frau Weber:** Auf Wiederhören!  

---

## 2. Redemittel

### Einladungen, Termine machen und ablehnen (making and declining appointments)

| German | English |
|---|---|
| Ich mache eine Party. | I'm having a party. |
| Ich lade dich / Sie ein. | I'm inviting you (informal / formal). |
| Hast du / Haben Sie am … Zeit? | Are you free on …? |
| Kannst du / Können Sie am … kommen? | Can you come on …? |
| Wann passt es dir / Ihnen? | When suits you? |
| Das passt (mir) gut / leider nicht. | That suits me / doesn't suit me, unfortunately. |
| Ich komme gern! | I'd love to come! |
| Ich kann leider nicht kommen. | I'm afraid I can't come. |
| Ich habe (leider) schon einen Termin. | I already have an appointment, unfortunately. |
| Ich schicke dir / Ihnen eine Einladung. | I'll send you an invitation. |

### Geburtstag und Feste (birthdays and celebrations)

| German | English |
|---|---|
| Herzlichen Glückwunsch zum Geburtstag! | Happy birthday! (warm, standard) |
| Alles Gute zum Geburtstag! | All the best for your birthday! |
| Wann hast du Geburtstag? | When is your birthday? |
| Ich habe am … Geburtstag. | My birthday is on the … |
| Frohe Weihnachten! | Merry Christmas! |
| Guten Rutsch! / Frohes Neues Jahr! | Happy New Year! |
| Frohe Ostern! | Happy Easter! |
| Schönes Wochenende! | Have a nice weekend! |

### Zeit und Datum (time and date)

| German | English |
|---|---|
| Welches Datum ist heute? | What's the date today? |
| Heute ist der erste März. | Today is the first of March. |
| Wann? / An welchem Tag? | When? / On which day? |
| Am Montag, dem dritten April. | On Monday, the third of April. |
| Im Januar / Im Sommer | In January / In summer |
| Um wie viel Uhr? | At what time? |
| Um acht Uhr / um halb neun | At eight o'clock / at half past eight |
| Vor dem Essen / nach dem Essen | Before the meal / after the meal |

---

## 3. Wortschatz

### Die Monate (months)

| | | | |
|---|---|---|---|
| der Januar | der April | der Juli | der Oktober |
| der Februar | der Mai | der August | der November |
| der März | der Juni | der September | der Dezember |

⚠️ All months are masculine (`der`). Learn them in order — they are the same in
German and English (with minor spelling differences). Plural: **die Monate**.

### Jahreszeiten und Feste (seasons and holidays)

| German | English | Wann? |
|---|---|---|
| der Frühling, -e | spring | März – Mai |
| der Sommer, - | summer | Juni – August |
| der Herbst, -e | autumn | September – November |
| der Winter, - | winter | Dezember – Februar |
| der Feiertag, -e | public holiday | — |
| der Geburtstag, -e | birthday | — |
| die Party, -s | party | — |
| das Fest, -e | celebration, festival | — |
| das Weihnachten | Christmas | 25.–26. Dezember |
| das Ostern | Easter | März/April |
| das Silvester | New Year's Eve | 31. Dezember |
| das Neujahr | New Year's Day | 1. Januar |
| der Karneval / der Fasching | carnival | Januar/Februar |
| der Termin, -e | appointment | — |
| die Einladung, -en | invitation | — |
| die Feier, -n | celebration | — |
| der Kalender, - | calendar | — |
| die Karte, -n | card, ticket | — |
| das Geschenk, -e | gift, present | — |

### Nützliche Verben

| Verb | English | Note |
|---|---|---|
| **feiern** | to celebrate | regular |
| **ein|laden** ⚠️ | to invite | separable: ich lade … ein |
| **schicken** | to send | regular |
| **gratulieren** | to congratulate | + Dativ: Ich gratuliere **dir**! |
| **passen** | to suit, to be convenient | + Dativ: Das passt **mir**. |
| **absagen** ⚠️ | to cancel, decline | separable |
| **planen** | to plan | regular |
| **statt|finden** ⚠️ | to take place | separable |
| **wünschen** | to wish | Ich wünsche dir alles Gute! |

> ✏️ **Selbsttest Wortschatz:** Cover the German column. Can you recall all words from memory? Aim for ≥80% before moving to the exercises.

---

## 4. Grammatik

### 4.1 Ordinal numbers (Ordinalzahlen)

Ordinal numbers are formed by adding **-te** (1st–19th) or **-ste** (20th+) to the
cardinal number, then adding an adjective ending. In dates you will mainly see
the **dative** ending `-en` (after `am`) and the **nominative** ending `-e` (after `der`).

| Cardinal | Ordinal stem | Example (nominative) | Example (dative after `am`) |
|---|---|---|---|
| 1 | erst- ⚠️ | der **erste** Mai | am **ersten** Mai |
| 2 | zweit- | der **zweite** März | am **zweiten** März |
| 3 | dritt- ⚠️ | der **dritte** Oktober | am **dritten** Oktober |
| 4 | viert- | der **vierte** Juli | am **vierten** Juli |
| 5 | fünft- | der **fünfte** Januar | am **fünften** Januar |
| 6 | sechst- | der **sechste** April | am **sechsten** April |
| 7 | siebt- ⚠️ | der **siebte** Februar | am **siebten** Februar |
| 8 | acht- ⚠️ | der **achte** August | am **achten** August |
| 9 | neunt- | der **neunte** Juni | am **neunten** Juni |
| 10 | zehnt- | der **zehnte** September | am **zehnten** September |
| 11–19 | + -te | der **elfte** November | am **elften** November |
| 20 | zwanzigst- | der **zwanzigste** Mai | am **zwanzigsten** Mai |
| 21 | einundzwanzigst- | der **einundzwanzigste** | am **einundzwanzigsten** |
| 31 | einunddreißigst- | der **einunddreißigste** | am **einunddreißigsten** |

⚠️ Irregular: **erst-** (not *eint-*), **dritt-** (not *dreit-*), **siebt-** (not
*sievent-*), **acht-** (no extra -t).

**Written dates:** In German, dates are written with a period after the number:
`1.` = der Erste, `3.` = der Dritte. A full date looks like:
`Montag, der 3. Oktober` (spoken: *Montag, der dritte Oktober*)
or in invitations: `am 3. Oktober` / `am dritten Oktober`.

> 📌 **Merkasten — Ordinalzahlen**
> | Range | Rule | Exceptions |
> |---|---|---|
> | 1–19 | stem + `-te` | **erst-**, **dritt-**, siebt-, acht- |
> | 20+ | stem + `-ste` | zwanzigst-, einundzwanzigst- … |
> 
> Used with `am` + Dativ ending: `am **ersten** Mai`, `am **zwanzigsten** März`.  
> ▶ Always add adjective endings after `am`: `am erst**en**`, `am zwanzigst**en**`.

### 4.2 Temporal prepositions: im, am, um, vor, nach

These five prepositions cover almost all time expressions you need at A1.

| Preposition | Used with | Examples |
|---|---|---|
| **im** | months, seasons, years | im Januar · im Sommer · im Jahr 2024 |
| **am** | days of the week, dates, parts of the day | am Montag · am 3. Mai · am Morgen, am Abend |
| **um** | clock times | um 8 Uhr · um halb neun · um Mitternacht |
| **vor** | "before" + dative noun | vor dem Essen · vor dem Kurs · vor dem Winter |
| **nach** | "after" + dative noun | nach dem Essen · nach der Party · nach Hause |

⚠️ **`am` + weekend:** Germans say `am Wochenende` (not *in*).

⚠️ **`am` + time of day:** `am Morgen, am Nachmittag, am Abend` — but `in der
Nacht` (exception!).

⚠️ Portuguese/Spanish interference: `em janeiro` = **im** Januar (not *in*);
`na segunda-feira` / `el lunes` = **am** Montag (not *in*).

> 📌 **Merkasten — Temporale Präpositionen**
> | Präposition | Use | Example |
> |---|---|---|
> | `um` | clock time | um **9 Uhr** |
> | `am` | day / date / part of day | am **Montag** · am **ersten Mai** · am **Abend** |
> | `im` | month / season | im **Juli** · im **Sommer** |
> | `vor` | before (time unit) | vor **zwei Wochen** |
> | `nach` | after (time unit) | nach **dem Essen** |
> 
> ▶ `um/am/im` are the three core ones — drill them until automatic.

### 4.3 Making and declining appointments — sentence patterns

**Making an appointment:**
> **Hast du am Freitag Zeit?**
> Kannst du am dritten Mai kommen?
> Wann passt es dir?

**Accepting:**
> Ja, das **passt** (mir) gut.
> Ich **komme** gern!

**Declining politely:**
> Nein, das geht **leider** nicht. — No, I'm afraid that doesn't work.
> Ich **kann** leider nicht kommen. — I'm afraid I can't come.
> Ich habe **schon** einen Termin. — I already have an appointment.

**Suggesting an alternative:**
> Geht es auch am Samstag? — Would Saturday work too?
> Was ist mit dem Zwanzigsten? — How about the 20th?

⚠️ `leider` (unfortunately) is the politeness softener that saves these
declinations from sounding rude. Learn it as part of the phrase.

> 📌 **Merkasten — Verabredungen: Schlüsselsätze**
> | Function | German |
> |---|---|
> | Accept | `Ja, das passt (mir) gut.` |
> | Decline (polite) | `Tut mir leid, da kann ich leider nicht.` |
> | Suggest alternative | `Wie wäre es mit Donnerstag um 15 Uhr?` |
> | Confirm | `Dann bis Donnerstag!` |
> 
> ▶ Always give a reason or alternative when declining — it sounds more natural and polite.

### 4.4 `einladen` — separable verb recap (L5)

`einladen` is separable: the prefix `ein-` detaches and goes to the end.

| | |
|---|---|
| Ich **lade** dich **ein**. | I'm inviting you. |
| Er **lädt** sie **ein**. ⚠️ | He is inviting her. (a→ä stem change!) |
| Wir **laden** alle **ein**. | We're inviting everyone. |
| Hast du sie **eingeladen**? | Did you invite her? (Perfekt) |

⚠️ `einladen` also has a vowel change: `a → ä` for `du` (lädst) and `er/sie/es`
(lädt). More a→ä verbs: `fahren`, `tragen`, `schlafen` (already known from L5–L6).

> 📌 **Merkasten — `einladen` (trennbar)**
> | Form | Example |
> |---|---|
> | Präsens | Ich **lade** dich **ein**. / Er **lädt** uns **ein**. (a→ä) |
> | Perfekt | Ich habe dich **eingeladen**. (ge- between prefix+stem) |
> | Imperativ (Sie) | **Laden** Sie mich **ein**! |
> 
> ▶ Partizip II of separable verbs: prefix + ge + stem → ein**ge**laden.

### 4.5 Pronunciation

- **ei** in `ein|laden, Einladung, Feiertag` = "eye" (same as in L1: `heißen`).
- **ie** in `viert-, siebt-` = "ee" sound.
- **ch** in `nach, Weihnachten` = the "ach-sound" (back of mouth, after `a/o/u`).
  Compare: **ich** (soft, front) vs. **nach** (hard, back).
- Ordinals ending in **-ten**: the `-t-` is clearly pronounced, so `dritten` ≠ `drüben`.
- `Feste, Feiertag, feiern`: the `F` is pronounced as English *f* (not *v*).

---

## 5. Lesetext: Ein Brief an die Familie

> Liebe Familie,
>
> ich lade euch herzlich zu meiner Geburtstagsfeier ein! Ich habe am Samstag,
> dem siebzehnten Juni, Geburtstag — ich werde 30! Die Feier findet am Abend statt:
> am achtzehnten Juni, einem Sonntag, um achtzehn Uhr. Die Party ist bei mir zu
> Hause, in der Mozartstraße 4.
>
> Bitte sagt mir bis zum zehnten Juni Bescheid, ob ihr kommen könnt.
>
> Ich freue mich auf euch!
>
> Viele Grüße,
> Anna

*(You'll work with this text in the exercises.)*

---

## 6. Hörtext

*Hör zu und mach Übung H3. Öffne das Transkript erst nach dem Hören!*

🎧 **Audio:** [hoertext.mp3](audio/hoertext.mp3)

<details>
<summary>📄 Transkript (erst nach dem Hören öffnen!)</summary>

> Bruno ruft Yuki an. Er möchte sie zur Geburtstagsfeier einladen.
> Yukis Geburtstag ist am dritten Juli. Bruno fragt: „Hast du am Samstag Zeit?"
> Yuki sagt: „Der zweite Juli passt mir leider nicht. Ich habe einen Termin."
> Bruno schlägt den vierten Juli vor. Yuki sagt: „Ja, der vierte passt gut!"
> Bruno freut sich und schickt ihr eine Einladung.

</details>

---

## 7. Lernstrategie

Dates and ordinals only become automatic through repetition in context. Every
morning for the next two weeks, open your phone's calendar and say today's date
out loud in German: *Heute ist Mittwoch, der achtzehnte Juni.* Then say your
birthday: *Ich habe am … Geburtstag.* Two sentences, every day — that's all it
takes to own the ordinals.

➡️ **Now do `exercises.md`, Block A first.**
