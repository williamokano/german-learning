---
level: A2
number: 7
slug: vergleichen
title: Vergleichen
titleEn: Comparing
canDo:
  - compare products, cities, and people
  - say what you prefer
  - describe statistics and trends
  - use comparative and superlative forms
grammar:
  - "comparative (schneller, teurer) and als / so … wie"
  - "superlative predicative (am schnellsten) and attributive (der schnellste)"
  - "irregular forms (gut → besser → am besten, viel, gern, hoch, nah)"
buildsOn:
  - A1/06 (gern/lieber introduction)
  - A1/03 (basic predicative adjectives)
  - A2/06 (verbs with prepositions)
---

# A2 · Lektion 7 — Vergleichen (Comparing)

> **You will learn to:** compare products, cities, and people; say what you prefer;
> describe statistics and trends; use the words for "faster than" and "as fast as".
>
> **Grammar:** comparative (`schneller`, `teurer`) · superlative predicative
> (`am schnellsten`) · superlative attributive (`der/die/das schnellste`) ·
> equality `so … wie` · inequality `… als` · irregular forms
> (`gut → besser → am besten`, `viel → mehr → am meisten`,
> `gern → lieber → am liebsten`).
>
> **Builds on:** adjectives predicative (L4), `gern/lieber` intro (A1/L6),
> modal verbs (A1/L7), cases Nom/Akk/Dat (A1/L3–L9),
> verbs with prepositions (A2/L6).

---

## 1. Dialog: Im Kaufhaus

🎧 **Audio:** [dialog1_a.mp3](audio/dialog1_a.mp3)

### Dialog A: Informell — du

> **Anna:** Schau mal, dieses rote Kleid! Es ist so schön!  
> **Bruno:** Ja, aber das blaue ist schöner, finde ich.  
> **Anna:** Wirklich? Das blaue ist teurer als das rote.  
> **Bruno:** Stimmt, aber Qualität kostet! Und welches gefällt dir besser?  
> **Anna:** Hmm … das rote ist günstiger, aber das blaue ist eleganter.  
> **Bruno:** Also ich würde das blaue nehmen. Es ist das eleganteste im ganzen Laden.  
> **Anna:** Okay, du hast mich überzeugt. Ich nehme das blaue!  

🎧 **Audio:** [dialog1_b.mp3](audio/dialog1_b.mp3)

### Dialog B: Formell — Sie

> **Frau Weber:** Guten Tag! Ich suche einen Laptop für meine Tochter. Welcher ist am besten?  
> **Herr Steinmeyer:** Guten Tag, Frau Weber. Das kommt darauf an. Dieser hier ist der günstigste — er kostet 449 Euro.  
> **Frau Weber:** Und der schwarze da? Er sieht moderner aus.  
> **Herr Steinmeyer:** Ja, der schwarze ist moderner und schneller als der graue, aber er ist auch teurer.  
> **Frau Weber:** Wie viel teurer?  
> **Herr Steinmeyer:** Ungefähr 150 Euro mehr. Aber er hat den besten Akku — sechs Stunden länger als das günstigere Modell.  
> **Frau Weber:** Das ist ein gutes Argument. Ich nehme lieber den schwarzen.  

---

## 2. Redemittel (ready-made phrases)

### Vergleichen (comparing)

| German | English |
|---|---|
| A ist schneller als B. | A is faster than B. |
| A ist so schnell wie B. | A is as fast as B. |
| A ist nicht so teuer wie B. | A is not as expensive as B. |
| A ist genauso gut wie B. | A is just as good as B. |
| A ist viel / ein bisschen besser als B. | A is much / a bit better than B. |
| A ist am schnellsten / am besten. | A is the fastest / the best. |
| A ist der/die/das schnellste. | A is the fastest (attributive). |

### Vorlieben ausdrücken (expressing preferences)

| German | English |
|---|---|
| Ich mag A lieber als B. | I like A better than B. |
| Ich finde A schöner als B. | I find A nicer than B. |
| Welches gefällt dir/Ihnen besser? | Which do you like better? |
| Das gefällt mir am besten. | I like this one the best. |
| Ich nehme lieber … | I would rather take … |
| Ich bevorzuge … | I prefer … |

### Statistiken und Trends (statistics and trends)

| German | English |
|---|---|
| A ist größer / kleiner als B. | A is bigger / smaller than B. |
| Die Preise steigen / sinken. | Prices are rising / falling. |
| Deutschland hat mehr Einwohner als Österreich. | Germany has more inhabitants than Austria. |
| Berlin ist die größte Stadt in Deutschland. | Berlin is the largest city in Germany. |
| Der Sommer war wärmer als letztes Jahr. | The summer was warmer than last year. |

---

## 3. Wortschatz (vocabulary)

### Adjektive für Vergleiche (adjectives for comparing)

| Adjektiv | English | Comparative | Superlative |
|---|---|---|---|
| schnell | fast | schneller | am schnellsten |
| langsam | slow | langsamer | am langsamsten |
| groß | big | größer ⚠️ | am größten |
| klein | small | kleiner | am kleinsten |
| teuer | expensive | teurer | am teuersten |
| günstig / billig | cheap / affordable | günstiger / billiger | am günstigsten / am billigsten |
| leicht | light / easy | leichter | am leichtesten |
| schwer | heavy / difficult | schwerer | am schwersten |
| neu | new | neuer | am neuesten |
| alt | old | älter ⚠️ | am ältesten |
| jung | young | jünger ⚠️ | am jüngsten |
| warm | warm | wärmer ⚠️ | am wärmsten |
| kalt | cold | kälter ⚠️ | am kältesten |
| modern | modern | moderner | am modernsten |
| elegant | elegant | eleganter | am elegantesten |
| laut | loud | lauter | am lautesten |
| ruhig | quiet | ruhiger | am ruhigsten |
| schön | beautiful | schöner | am schönsten |
| gut | good | besser ⚠️ | am besten ⚠️ |
| viel | much / many | mehr ⚠️ | am meisten ⚠️ |
| gern | gladly (like to) | lieber ⚠️ | am liebsten ⚠️ |

⚠️ Umlaut changes: many one-syllable adjectives add an umlaut in comparative and superlative (alt → älter, warm → wärmer, groß → größer, jung → jünger, kalt → kälter).

### Nomen für Statistiken und Einkaufen

| Noun (with article + plural) | English |
|---|---|
| der Preis, -e | price |
| das Modell, -e | model |
| die Qualität, -en | quality |
| der Akku, -s | battery (rechargeable) |
| der Vergleich, -e | comparison |
| die Einwohnerzahl, -en | population count |
| der Einwohner, - | inhabitant (m) |
| die Einwohnerin, -nen | inhabitant (f) |
| die Fläche, -n | area, surface |
| die Entfernung, -en | distance |
| der Unterschied, -e | difference |
| die Ähnlichkeit, -en | similarity |
| der Vorteil, -e | advantage |
| der Nachteil, -e | disadvantage |

### Verben und Ausdrücke (verbs and expressions)

| Verb / Expression | English | Notes |
|---|---|---|
| vergleichen | to compare | hat verglichen ⚠️ |
| bevorzugen | to prefer | hat bevorzugt |
| übertreffen | to surpass, exceed | hat übertroffen ⚠️ |
| steigen | to rise, increase | ist gestiegen ⚠️ |
| sinken | to fall, decrease | ist gesunken ⚠️ |
| es kommt darauf an | it depends | fixed phrase |

> ✏️ **Selbsttest Wortschatz:** Cover the German column. Can you recall all words from memory? Aim for ≥80% before moving to the exercises.

---

## 4. Grammatik

### 4.1 Comparative — forming it

Add **`-er`** to the adjective stem, then add any required adjective ending:

| Adjektiv | Komparativ |
|---|---|
| schnell | schnell**er** |
| teuer | teur**er** ⚠️ (drops the inner -e-) |
| modern | modern**er** |
| leicht | leicht**er** |

**Predicative comparative** (after `sein`, `werden`, `finden`) — no extra ending:

> Das Auto ist **schneller** als das Fahrrad.
> Diese Wohnung ist **teurer** als die andere.
> Ich finde Berlin **interessanter** als München.

**`als`** introduces the second element of an unequal comparison:

> Berlin ist größer **als** Wien.
> Er läuft schneller **als** ich.

⚠️ Never use `wie` for unequal comparisons: *Berlin ist größer **wie** Wien* ✗

> 📌 **Merkasten — Komparativ**
> | Form | Pattern | Example |
> |---|---|---|
> | regular | Adj + **-er** | klein → kleiner |
> | umlaut | Adj-umlaut + **-er** | groß → größer |
> | irregular | must memorize | gut → **besser** · viel → **mehr** · gern → **lieber** |
> 
> ▶ Always use `als` after a comparative: `Berlin ist größer **als** München.`

---

### 4.2 Equality — `so … wie`

For equal comparisons, use **`so + adjective + wie`**:

> München ist **so** schön **wie** Berlin.
> Das blaue Kleid ist **genauso** teuer **wie** das rote.
> Er spricht Deutsch **so** gut **wie** Englisch.

To negate: **`nicht so … wie`** = not as … as:

> Paris ist **nicht so** groß **wie** London.

⚠️ `als` = unequal (more/less than) · `wie` = equal (as … as). This is a very common error — memorize the rule.

> 📌 **Merkasten — Gleichheit**
> | Structure | Meaning | Example |
> |---|---|---|
> | `so [Adj] wie` | as … as | so groß wie Berlin |
> | `genauso [Adj] wie` | just as … as | genauso teuer wie München |
> | `nicht so [Adj] wie` | not as … as | nicht so warm wie im Süden |

---

### 4.3 Superlative — predicative form

Add **`-sten`** to the stem: **`am ___sten`**

| Adjektiv | Superlativ (prädikativ) |
|---|---|
| schnell | am schnell**sten** |
| modern | am modern**sten** |
| langsam | am langsam**sten** |

**Spelling rules:**
- Stems ending in **-d, -t, -s, -ß, -x, -z**: insert an extra `-e-` before `-sten`:
  `neu → am neu**esten**`, `laut → am laut**esten**`, `alt → am ält**esten**`.
- Stems with an umlaut in comparative keep it in superlative: `groß → größer → am größ**ten**` (the ß + t merge as -st).

> Dieses Hotel ist **am teuersten**.
> Im Sommer ist es hier **am heißesten**.
> Welche Stadt ist **am größten**?

> 📌 **Merkasten — Superlativ prädikativ**
> | Form | Pattern | Example |
> |---|---|---|
> | regular | `am` + Adj + **-sten** | am kleinsten |
> | Adj ending in `-t`, `-d`, `-s`, `-ß` | add **-esten** | am größten · am lautesten |
> | irregular | must memorize | gut → **am besten** · viel → **am meisten** · gern → **am liebsten** |

---

### 4.4 Superlative — attributive form (declined!)

When the superlative is used **before a noun**, it takes full adjective endings. This is the most common learner trap.

**Formula:** `der/die/das + stem + -ste + ending` (after definite article → weak declension)

| Case | Masculine | Feminine | Neuter | Plural |
|---|---|---|---|---|
| Nom. | der schnellst**e** | die schnellst**e** | das schnellst**e** | die schnellst**en** |
| Akk. | den schnellst**en** | die schnellst**e** | das schnellst**e** | die schnellst**en** |
| Dat. | dem schnellst**en** | der schnellst**en** | dem schnellst**en** | den schnellst**en** |

Examples:

> Das ist der **schnellste** Zug in Deutschland. (Nom. m)
> Ich nehme das **günstigste** Modell. (Akk. n)
> Wir wohnen in der **ruhigsten** Straße. (Dat. f)
> Er hat die **besten** Freunde der Welt. (Akk. pl)

⚠️ **The attributive superlative always needs an ending!** You cannot write *der schnellst* — the ending is mandatory. Compare:

> Das Auto ist am schnellsten. ✓ (predicative — no noun follows)
> Das ist das schnellste Auto. ✓ (attributive — ending -e because neuter Nom.)
> Das ist das schnellst Auto. ✗

> 📌 **Merkasten — Superlativ attributiv**
> The attributive superlative = `[article] + [Adj]-st- + ending`:
> - `der/die/das` + **größt-** + weak ending → `der größte Berg`
> - Endings follow the same weak-declension table as regular adjectives.
> 
> ▶ Always has an article (no bare superlative adjective without `der/die/das`).

---

### 4.5 Irregular forms — must memorize!

| Base | Comparative | Superlative (pred.) | Superlative (attr.) |
|---|---|---|---|
| **gut** | **besser** | **am besten** | der/die/das **beste** |
| **viel** | **mehr** | **am meisten** | der/die/das **meiste** |
| **gern** | **lieber** | **am liebsten** | — (adverb only) |
| **hoch** | **höher** ⚠️ | **am höchsten** | der/die/das **höchste** |
| **nah** | **näher** | **am nächsten** | der/die/das **nächste** |

⚠️ **`hoch`** drops the `-ch-` spelling pattern: `hoch → höher` (not *hocher*). The comparative stem is `höh-`.

> Das ist die **beste** Lösung. · Er kann **mehr** essen als ich.
> Ich esse **am liebsten** Pizza. · Das höchste Gebäude in Berlin ist der Fernsehturm.

---

### 4.6 Pronunciation: comparative endings

- The comparative `-er` suffix is a **reduced vowel (Schwa)**: `schöner` ≈ "SHEU-ner" (like the English "-er" in "better").
- Don't add a second syllable where there is none: `teuer` already ends in `-er`, so comparative is `teurer` (teu-**rer**) — not *teuerer*.
- Attributive superlative: `-sten` or `-esten` is one short syllable: `schnellsten` ≈ "SCHNELL-sten".

---

## 5. Lesetext: Berlin und München — ein Vergleich

> Berlin und München sind die bekanntesten Städte Deutschlands — aber sie sind sehr verschieden.
>
> Berlin ist die größte Stadt: Es hat etwa 3,7 Millionen Einwohner. München hat
> nur etwa 1,5 Millionen, aber München ist die reichste Großstadt Deutschlands —
> die Gehälter sind hier am höchsten. Das Leben in München ist teurer als in Berlin:
> Eine Wohnung kostet in München fast doppelt so viel wie in Berlin.
>
> Dafür hat Berlin mehr Museen, Clubs und Grünflächen. Viele junge Menschen finden
> Berlin cooler und interessanter als München. Viele Touristen finden Berlin
> die aufregendste Hauptstadt Europas — ob das stimmt, ist natürlich Geschmackssache.
>
> Das Wetter ist in beiden Städten ähnlich, aber München ist im Winter etwas kälter
> als Berlin. Dafür ist die Luft in München klarer als in der Hauptstadt.
>
> Was ist besser — Berlin oder München? Das kommt darauf an! Für Kultur und
> Nachtleben ist Berlin am besten. Für Natur und höhere Gehälter ist München
> die bessere Wahl.

*(You'll work with this text in the exercises — Block C, Lesen.)*

---

## 6. Hörtext

*Hör zu und mach Übung H3. Öffne das Transkript erst nach dem Hören!*

🎧 **Audio:** [hoertext.mp3](audio/hoertext.mp3)

<details>
<summary>📄 Transkript (erst nach dem Hören öffnen!)</summary>

> Yuki wohnt in München und macht gerade einen Deutschkurs. Gestern hat sie mit ihrer Freundin Hana über Smartphones gesprochen. Yukis Handy ist älter als Hanas Handy, aber es funktioniert noch gut. Hanas neues Modell ist schneller und hat einen besseren Akku. Es kostet aber fast dreimal so viel wie Yukis Handy. Am Ende hat Yuki entschieden: Sie kauft vorerst kein neues Handy — ihr altes ist gut genug!

</details>

---

## 7. Lernstrategie

**The "comparison chain" technique:** Pick three things you know in German (cities, foods, objects, people) and write one comparison sentence for each structure you've learned today: *A ist [Komp.] als B · A ist so [Adj.] wie B · A ist am [Superl.]* Write them on a card and say them out loud until they feel automatic. Tomorrow, swap the objects. This forces you to apply the endings in fresh contexts — the most reliable way to prevent the common *als/wie* swap.

➡️ **Now do `exercises.md`. Check yourself with `solutions.md` only afterwards.**
