---
level: A2
number: 9
slug: hoeflichkeit-und-wuensche
title: Höflichkeit und Wünsche
titleEn: Politeness and Wishes
canDo:
  - make polite requests in a restaurant, shop, and hotel
  - accept and decline invitations politely
  - make a polite complaint or apology
  - express wishes using Ich hätte gern …
grammar:
  - "Konjunktiv II — würde + Infinitiv (polite requests/offers)"
  - "könnte (could), hätte (would have / would like to have), wäre (would be)"
  - "register: polite Konjunktiv II vs. direct commands"
buildsOn:
  - A1/07 (modal verbs können, wollen, möchten)
  - A1/10 (imperative)
  - A2/04 (conditional clauses with wenn)
---

# A2 · Lektion 9 — Höflichkeit & Wünsche (Politeness & Wishes)

> **You will learn to:** make polite requests in a restaurant, shop, and hotel;
> accept and decline invitations politely; make a polite complaint or apology;
> express wishes and hypothetical preferences; use the chunk `Ich hätte gern…`
> to order food or items.
>
> **Grammar:** Konjunktiv II — `würde + Infinitiv` (polite requests/offers) ·
> `könnte` (could) · `hätte` (would have) · `wäre` (would be) — conjugation
> tables + practical use in real situations.
>
> **Builds on:** modal verbs `können`, `wollen`, `möchten` (A1/L6–L7) ·
> dative pronouns (A1/L9) · food & restaurant vocab (A1/L3) · imperative
> (A1/L10) · conditional clauses with `wenn` (A2/L4) · adjective declension
> (A2/L8).

---

## 1. Dialog: Im Restaurant und anderswo

🎧 **Audio:** [dialog1_a.mp3](audio/dialog1_a.mp3)

### Dialog A: Informell — du

> **Anna:** Bruno, ich hätte Lust, heute Abend essen zu gehen. Hast du Zeit?  
> **Bruno:** Ja, gerne! Wohin würdest du gerne gehen?  
> **Anna:** Ich würde gern ins neue italienische Restaurant in der Mitte gehen. Was denkst du?  
> **Bruno:** Klingt super. Ich käme auch gerne mit — ich könnte schon um sieben da sein.  
> **Anna:** Perfekt! Ich würde dann online einen Tisch reservieren.  
> **Bruno:** Könntest du bitte einen Fensterplatz anfragen? Die Aussicht soll toll sein.  
> **Anna:** Klar, mache ich. Ach, und könntest du die Adresse kurz googeln? Ich finde sie gerade nicht.  
> **Bruno:** Natürlich. Hier: Goethestraße 12. Wäre es okay, wenn wir mit der U-Bahn fahren?  
> **Anna:** Ja, das wäre prima!  

🎧 **Audio:** [dialog1_b.mp3](audio/dialog1_b.mp3)

### Dialog B: Formell — Sie

> **Kellner:** Guten Abend! Haben Sie reserviert?  
> **Herr Steinmeyer:** Ja. Steinmeyer, für zwei Personen. Wir hätten gern einen ruhigen Tisch, wenn möglich.  
> **Kellner:** Natürlich, einen Moment bitte. — Hier, bitte sehr. Darf ich Ihnen die Karte bringen?  
> **Frau Weber:** Ja, bitte. Könnten Sie mir auch sagen, was das Tagesgericht ist?  
> **Kellner:** Heute empfehlen wir gegrillten Lachs mit Gemüse. Das wäre sehr lecker!  
> **Herr Steinmeyer:** Ich hätte gern den Lachs, bitte.  
> **Frau Weber:** Und ich hätte gern das vegetarische Pasta-Gericht. Könnten Sie die Soße bitte ohne Knoblauch machen?  
> **Kellner:** Aber natürlich. Würden Sie auch etwas trinken möchten?  
> **Frau Weber:** Ja — ich hätte gern ein Glas Weißwein, bitte.  
> **Herr Steinmeyer:** Und ich hätte gern eine Flasche Mineralwasser.  
> **Kellner:** Sehr gerne. Ich bringe das sofort.  

---

## 2. Redemittel (ready-made phrases)

### Polite requests

| German | English | Register |
|---|---|---|
| Könnten Sie / Könntest du … bitte …? | Could you please …? | formal / informal |
| Würden Sie … bitte …? | Would you please …? | formal |
| Würdest du … bitte …? | Would you please …? | informal |
| Dürfte ich … ? | Might I …? | very formal |
| Wäre es möglich, … zu …? | Would it be possible to …? | formal |
| Ich würde gern … | I would like to … | neutral |

### Ordering and expressing preferences

| German | English |
|---|---|
| Ich hätte gern … | I would like … (ordering food/items) |
| Ich hätte gern einen Kaffee, bitte. | I'd like a coffee, please. |
| Könnten Sie mir bitte … bringen? | Could you please bring me …? |
| Was würden Sie empfehlen? | What would you recommend? |
| Ich würde lieber … nehmen. | I would rather take … |

### Invitations — accepting and declining

| Function | German | English |
|---|---|---|
| Invite | Würdest du / Würden Sie gern …? | Would you like to …? |
| Accept | Ja, das wäre toll! / Gerne! | Yes, that would be great! / With pleasure! |
| Accept | Ja, ich käme gerne! | Yes, I'd love to come! |
| Decline (politely) | Das wäre leider nicht möglich. | That unfortunately wouldn't be possible. |
| Decline (politely) | Ich hätte leider keine Zeit. | I unfortunately wouldn't have time. |
| Decline with alternative | Ich könnte nicht am Freitag, aber am Samstag wäre es gut. | I couldn't on Friday, but Saturday would work. |

### Polite complaints and apologies

| German | English |
|---|---|
| Entschuldigung, ich glaube, das wäre ein Irrtum. | Excuse me, I think there might be a mistake. |
| Könnten Sie das bitte noch einmal prüfen? | Could you please check that again? |
| Ich hätte eigentlich … bestellt. | I had actually ordered … |
| Es tut mir leid — das hätte nicht passieren sollen. | I'm sorry — that shouldn't have happened. |
| Das wäre sehr nett von Ihnen. | That would be very kind of you. |

### Expressing wishes

| German | English |
|---|---|
| Ich würde gern … | I would like to … |
| Ich wünschte, ich hätte mehr Zeit. | I wish I had more time. |
| Es wäre schön, wenn … | It would be nice if … |
| Ich hätte gern … (+ noun) | I'd like to have … |
| Das wäre mein Traum! | That would be my dream! |

---

## 3. Wortschatz (vocabulary)

### Im Restaurant (at the restaurant)

| Noun (with article + plural) | English |
|---|---|
| die Speisekarte, -n | menu |
| das Tagesgericht, -e | dish of the day, daily special |
| die Vorspeise, -n | starter |
| das Hauptgericht, -e | main course |
| die Nachspeise, -n / der Nachtisch, -e | dessert |
| die Rechnung, -en | bill |
| das Trinkgeld, -er | tip |
| der Kellner, - / die Kellnerin, -nen | waiter / waitress |
| der Platz, –e (Plätze) | seat, place |
| der Fensterplatz, –e (Fensterplätze) | window seat |
| die Reservierung, -en | reservation |
| die Bestellung, -en | order |
| die Soße, -n | sauce |
| das Gericht, -e | dish, meal |

### Im Geschäft (at the shop)

| Noun (with article + plural) | English |
|---|---|
| der Umtausch (no pl.) | exchange (of goods) |
| die Quittung, -en | receipt |
| die Reklamation, -en | complaint (formal, about goods) |
| der Kassenbon, -s | cash receipt |
| der Artikel, - | item, article |
| die Größe, -n | size |
| das Angebot, -e | offer, special deal |

### Einladungen (invitations)

| Noun (with article + plural) | English |
|---|---|
| die Einladung, -en | invitation |
| die Verabredung, -en | date, appointment (social) |
| der Vorschlag, –e (Vorschläge) | suggestion, proposal |
| die Absage, -n | cancellation, decline |
| die Zusage, -n | acceptance, confirmation |

### Wichtige Verben (important verbs)

| Verb | English | Note |
|---|---|---|
| empfehlen ⚠️ | to recommend | er empfiehlt (e→ie) |
| bestellen | to order | regular |
| reservieren | to reserve | regular, -ieren |
| reklamieren | to complain formally | regular, -ieren |
| ablehnen | to decline, refuse | separable |
| annehmen ⚠️ | to accept | separable, er nimmt an |
| einladen ⚠️ | to invite | separable, er lädt ein |
| sich entschuldigen | to apologize | reflexive |

### Pronunciation note: Konjunktiv II vowels

The Konjunktiv II forms `wäre`, `hätte`, `käme` have an Umlaut (ä). In spoken German these vowels are clearly marked:

- **wäre** ≈ "VEH-reh" (ä like 'e' in "bed")
- **hätte** ≈ "HEH-teh"
- **käme** ≈ "KAY-meh"
- **könnte** ≈ "KÖNN-teh" (ö rounded, like saying "uh" with rounded lips)

This Umlaut is what signals you are speaking hypothetically / politely — don't drop it!

> ✏️ **Selbsttest Wortschatz:** Cover the German column. Can you recall all words from memory? Aim for ≥80% before moving to the exercises.

---

## 4. Grammatik

### 4.1 What is Konjunktiv II?

German has two subjunctive moods. **Konjunktiv II** is the one you'll use every day for:

1. **Polite requests** — "Could you…?", "Would you mind…?"
2. **Wishes and hypotheticals** — "I would like to…", "It would be nice if…"
3. **Polite offers** — "Would you like…?"

> ⚠️ **Scope note:** The full Konjunktiv II system — wishes about unreal conditions
> (`Wenn ich mehr Geld hätte, würde ich eine Weltreise machen`), all verbs' own
> Konjunktiv II forms, and the past Konjunktiv II (`hätte gemacht`) — belongs to
> **B1/03**. In this lesson we focus on four key forms used in everyday politeness.

---

### 4.2 `würde + Infinitiv` — the standard polite form

The most versatile Konjunktiv II tool. `würde` is the Konjunktiv II of `werden`
and works like English "would". Combine it with any infinitive:

**Conjugation of `würde`:**

| Pronoun | Form |
|---|---|
| ich | **würde** |
| du | **würdest** |
| er / sie / es | **würde** |
| wir | **würden** |
| ihr | **würdet** |
| sie / Sie | **würden** |

**Word order: `würde` takes position 2, infinitive goes to the END (Satzklammer).**

| Position 1 | Position 2 | Middle | End |
|---|---|---|---|
| Ich | **würde** | gern | kommen. |
| Würdest | **du** | bitte | helfen? |
| Wir | **würden** | gern | reservieren. |

> Ich **würde** gern ins Restaurant **gehen**.  
> **Würdest** du bitte die Rechnung **bringen**?  
> Was **würden** Sie empfehlen?

> 📌 **Merkasten — `würde + Infinitiv`**
> | Person | Form |
> |---|---|
> | ich / er / sie / es | **würde** |
> | du | **würdest** |
> | wir / sie / Sie | **würden** |
> | ihr | **würdet** |
> 
> `Ich **würde** gern kommen.` · `Was **würdest** du machen?`  
> ▶ Most common Konjunktiv II in everyday speech — use this as your default.

---

### 4.3 `könnte` — polite "could"

`könnte` is the Konjunktiv II of `können`. Use it instead of `würde + können` for
**polite requests** — it sounds natural and is very common in German.

**Conjugation of `könnte`:**

| Pronoun | Form |
|---|---|
| ich | **könnte** |
| du | **könntest** |
| er / sie / es | **könnte** |
| wir | **könnten** |
| ihr | **könntet** |
| sie / Sie | **könnten** |

> ⚠️ **Don't confuse:** `konnte` (Präteritum: he *could* in the past) vs. `könnte`
> (Konjunktiv II: he *could* — politely / hypothetically). The Umlaut ö makes all
> the difference!

> **Könntest** du mir bitte helfen?  
> **Könnten** Sie mir die Rechnung bringen?  
> Ich **könnte** um sieben da sein.

> 📌 **Merkasten — `könnte`**
> | Person | Form |
> |---|---|
> | ich / er / sie / es | **könnte** |
> | du | **könntest** |
> | wir / sie / Sie | **könnten** |
> | ihr | **könntet** |
> 
> `**Könnten** Sie mir helfen?` (polite request, formal)  
> `**Könntest** du mir helfen?` (polite request, informal)

---

### 4.4 `hätte` — polite "would have" / "would like to have"

`hätte` is the Konjunktiv II of `haben`. Its main use at A2 level:

1. **`Ich hätte gern…`** — the standard way to order food or items politely
   (chunk: learn as a fixed phrase)
2. **Polite expressing of wishes:** "I would have liked…", "I'd prefer…"
3. **Declining invitations:** "I unfortunately wouldn't have time."

**Conjugation of `hätte`:**

| Pronoun | Form |
|---|---|
| ich | **hätte** |
| du | **hättest** |
| er / sie / es | **hätte** |
| wir | **hätten** |
| ihr | **hättet** |
| sie / Sie | **hätten** |

> Ich **hätte** gern einen Kaffee, bitte.  
> **Hättest** du gern noch etwas Dessert?  
> Wir **hätten** gern einen Tisch für vier.

**Chunk alert — `Ich hätte gern…`:** This is the standard German phrase for
ordering. Think of it as "I would like…" in a restaurant or shop. You'll learn
the full mechanics of `hätte` at B1/03, but this chunk is safe to use now.

> 📌 **Merkasten — `hätte`**
> | Person | Form |
> |---|---|
> | ich / er / sie / es | **hätte** |
> | du | **hättest** |
> | wir / sie / Sie | **hätten** |
> | ihr | **hättet** |
> 
> `Ich **hätte** gern einen Kaffee.` (I'd like a coffee — polite)  
> `**Hätten** Sie einen Moment Zeit?` (formal polite question)

---

### 4.5 `wäre` — polite "would be"

`wäre` is the Konjunktiv II of `sein`. Use it to describe a hypothetical or
polite situation:

**Conjugation of `wäre`:**

| Pronoun | Form |
|---|---|
| ich | **wäre** |
| du | **wärst** |
| er / sie / es | **wäre** |
| wir | **wären** |
| ihr | **wäret** / **wärt** |
| sie / Sie | **wären** |

> Das **wäre** sehr nett.  
> Freitag **wäre** mir besser.  
> **Wäre** das in Ordnung für Sie?  
> Es **wäre** schön, wenn wir Zeit hätten.

> 📌 **Merkasten — `wäre`**
> | Person | Form |
> |---|---|
> | ich / er / sie / es | **wäre** |
> | du | **wärst** |
> | wir / sie / Sie | **wären** |
> | ihr | **wärt** |
> 
> `Das **wäre** sehr nett.` · `**Wäre** es möglich, …?`  
> ▶ Konjunktiv II of **sein** — use for hypothetical states and polite suggestions.

---

### 4.6 Quick comparison: which form to use?

| Situation | Best choice | Example |
|---|---|---|
| Polite request with any verb | `würde + Infinitiv` | Würdest du das bitte machen? |
| Polite request "could you" | `könnte` | Könnten Sie mir helfen? |
| Ordering food/drinks | `hätte gern` | Ich hätte gern einen Tee. |
| Describing hypothetical situation | `wäre` | Das wäre toll! |
| Saying "I could" (politely) | `könnte` | Ich könnte morgen kommen. |

---

### 4.7 Polite requests vs. direct commands — the register shift

Compare these three ways to ask for the same thing:

| Style | German | Feeling |
|---|---|---|
| Command (imperative) | Bringen Sie die Rechnung! | very direct, borderline rude |
| Direct request (`können` Präsens) | Können Sie die Rechnung bringen? | neutral, fine in everyday life |
| Polite Konjunktiv II | Könnten Sie bitte die Rechnung bringen? | softer, warmer, more elegant |

In a restaurant or formal setting, always prefer the Konjunktiv II form —
Germans notice and appreciate it. In informal settings with friends, `würdest du`
is friendly without being cold.

---

## 5. Lesetext: Anna lädt Bruno ein

> Anna hatte eine Idee. Sie wollte Bruno zu einem Konzert einladen — ein russisches Kammerorchester spielte in der Berliner Philharmonie. „Das wäre doch schön!", dachte sie.
>
> Sie schrieb Bruno eine Nachricht: „Hey! Ich hätte eine Frage: Würdest du am Freitagabend gern ins Konzert kommen? Es wäre um 20 Uhr in der Philharmonie. Ich hätte zwei Karten — ich hätte die Karten eigentlich mit meiner Schwester anschauen wollen, aber sie hat leider abgesagt."
>
> Bruno antwortete schnell: „Das wäre super! Ich käme sehr gerne. Könnte ich einen Platz reservieren, oder hast du das schon gemacht?" Anna schrieb zurück: „Ich habe schon reserviert — du musst nichts machen! Könntest du aber bitte das Restaurant danach aussuchen? Ich wäre froh, wenn du eine gute Idee hättest."
>
> Bruno suchte sofort nach Restaurants in der Nähe der Philharmonie. Er fand ein kleines italienisches Restaurant. „Das wäre perfekt", schrieb er. „Ich würde einen Tisch für halb zehn reservieren. Würde das passen?" — „Ja, das wäre ideal!", antwortete Anna. „Ich freue mich schon!"

*(You'll work with this text in the exercises — Block C, Lesen.)*

---

## 6. Hörtext

*Hör zu und mach Übung H3. Öffne das Transkript erst nach dem Hören!*

🎧 **Audio:** [hoertext.mp3](audio/hoertext.mp3)

<details>
<summary>📄 Transkript (erst nach dem Hören öffnen!)</summary>

> Yuki ist in München und geht heute in ein Café. Sie setzt sich an einen Tisch und ruft die Kellnerin. Die Kellnerin kommt und fragt: „Was darf es sein?" Yuki antwortet: „Ich hätte gern einen Cappuccino und ein Stück Apfelkuchen, bitte." Die Kellnerin bringt die Bestellung, aber Yuki bemerkt, dass der Kuchen Sahne hat — und sie mag keine Sahne. Yuki sagt höflich: „Entschuldigung, könnten Sie bitte den Kuchen ohne Sahne bringen? Das wäre sehr nett." Die Kellnerin lächelt und wechselt den Teller sofort. Yuki ist sehr zufrieden.

</details>

---

## 7. Lernstrategie

**"The polite upgrade" technique:** Every time you catch yourself using a direct
form (`Gib mir…`, `Ich will…`, `Können Sie…?`), immediately rephrase it with
Konjunktiv II. This forces your brain to practise the forms in real context.
Keep a small notebook this week and write down every polite request you make in
German — in the shop, at a restaurant, in messages to German-speaking friends.
After 7 days you will have activated `würde`, `könnte`, `hätte`, and `wäre`
across dozens of real situations, which is worth ten times more than a
conjugation drill alone.

➡️ **Now do `exercises.md`. Check yourself with `solutions.md` only afterwards.**
