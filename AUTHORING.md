...
**Play-button behaviour is configured centrally, not per lesson.** Lesson-body
players (dialogs/Hörtext) get a progress bar, seek (±5 s) and an unlimited
restart button; Hören **exercise** players get a restart button capped at
**2 replays** and no seeking (exam-style). The config is the `data-*` attributes
emitted in `web/src/plugins/remark-audio-links.mjs` (lesson body) and the
`<audio-play>` tags in `web/src/components/ExerciseShell.svelte`,
`SingleChoice.svelte` and `TrueFalse.svelte` (exercises) — see the attribute
reference at the top of `web/src/scripts/audio-play.js`.
