...
Add a `### 🔊 Aussprache` sub-section inside `## 3. Wortschatz`. One `Hör zu` line
per thematic group, items separated by ` · `:
...
The generator reads each group, produces one MP3 with 400 ms pauses between items
(clear announcer voice, no background noise), and patches lesson.md with a 🎧 link.
Use Tier 1 for: greetings, alphabet, numbers, core single-word vocabulary.
Stop at A1/04 — by then pronunciation is assumed to be established.

**Tier 2 — Dialogue audio (all lessons, all levels)**

The model dialogues already use `> **Speaker:** Text` blockquote format. No extra
markup needed. The generator auto-detects `## 1.` / `## 2.` Dialog headers in
lesson.md, collects the speaker turns, and produces `dialog1.mp3` / `dialog2.mp3`
with per-speaker voices and context-appropriate background ambience.

**Tier 3 — Hörtext (small story with Lückentext, all lessons, all levels)**

Every lesson ends with a short `## 6. Hörtext` section: a 4–6 sentence story on
the lesson topic, using **only** known material. The generator reads it aloud
with a single clear announcer voice, 1.2 s pauses between sentences, no
background, and produces `hoertext.mp3`. The audio is reused in Block H as a
**Hörtext-Lückentext**: the student listens and fills in 5–8 missing content
words (nouns, verbs, key adjectives) from a small word bank.

```
## 6. Hörtext

> Anna kommt aus Österreich. Sie wohnt jetzt in Berlin. Sie spricht
> Deutsch und Englisch. Ihr Deutschkurs hat zwanzig Studenten. Die
> Lehrerin heißt Frau Schmidt.
```

If a lesson has 2 dialogs you can keep the Hörtext simple and short — the
duolingo-style Lückentext is the main learning payoff. The Hörtext is a peer of
the Lesetext (which is used in C4 Lesen); the two are separate on purpose.
...
Open with the block table + advice to spread blocks over several days (copy the
pattern from A1/03).

| Block | Content | Size |
|---|---|---|
| **H — Hören** | Up to three exercise **types** (mix as fits the lesson): (1) **Dialog Hör-Check** — one exercise per `dialog<n>.mp3` (or one combined for multi-dialog lessons), 2–4 R/F or single-choice comprehension items each; (2) **Pronunciation check** (Tier 1 lessons A1/01–04 only) — match-audio-to-picture / circle-the-word-you-hear, anchored on the `Hör zu` clips; (3) **Hörtext-Lückentext** — the student listens to `hoertext.mp3` and fills 5–8 gaps from a small word bank. Block H comes **before** Block A. Not counted in the 28–32 target. | 1–3 exercises |
...
- Mirrors every exercise number, block by block — including Block H before Block A.
...
- [ ] Block H present in exercises.md and solutions.md (1–3 Hören exercises)
- [ ] Tier 1 only (A1/01–04): `### 🔊 Aussprache` section with `Hör zu` lines in lesson.md Wortschatz
- [ ] Tier 3 (all lessons): `## 6. Hörtext` section in lesson.md with a 4–6 sentence blockquote
