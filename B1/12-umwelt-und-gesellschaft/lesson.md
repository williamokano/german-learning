# B1 · Lektion 12 — Umwelt und Gesellschaft (Environment and Society)

> **You will learn to:** discuss environmental and social issues, express and
> justify opinions (Stellungnahme), use nominalized nouns and verb-to-noun
> transformations (Nominalisierung), and deploy Passiv in news/environment
> register.
>
> **Grammar:** Nominalisierung — `das Recyceln`, `die Verbesserung`, `die
> Möglichkeit`; Verbalstil vs. Nominalstil transformation; Passiv review in
> environmental and news context.
>
> **Builds on:** B1/04 (Passiv — central to news/environment language) ·
> B1/10 (Zweiteilige Konnektoren — for structured arguments) ·
> B1/02 (obwohl / trotzdem / deshalb)

---

## 1. Dialoge

### Dialog A: Die Citymaut — Mia, Lukas und Pavel diskutieren (informal, 3 Personen)

*Scene: Mia and Lukas's kitchen in Kreuzberg. Saturday morning. Mia has been
reading a newspaper article about Berlin's new congestion-charge (Citymaut)
plan. Pavel has come over for coffee. The three debate the pros and cons.*

> **Mia:** Habt ihr das heute gelesen? Berlin plant, nächstes Jahr eine Citymaut einzuführen. Autos zahlen ab dem ersten Kilometer innerhalb des S-Bahn-Rings fünf Euro pro Tag.  
> **Lukas:** Endlich! Der Schutz der Luft in der Innenstadt ist längst überfällig. Die Reduzierung des Autoverkehrs wird die Emissionen deutlich senken.  
> **Pavel:** Hm. Ich verstehe den Gedanken, aber die Nutzung eines Autos ist für viele Leute keine Frage des Komforts, sondern der Notwendigkeit. Was ist mit Handwerkern, die Werkzeug transportieren müssen?  
> **Mia:** Das stimmt. Einerseits ist die Verbesserung der Luftqualität wichtig — das bezweifelt niemand. Andererseits trifft die Citymaut kleine Betriebe viel härter als große Firmen.  
> **Lukas:** Das Argument kenne ich. Aber die Daten zeigen, dass nicht nur die Luft, sondern auch der Lärm massiv abnimmt, wenn weniger Autos fahren. Außerdem soll ein Teil der Einnahmen in den Ausbau des ÖPNV investiert werden.  
> **Pavel:** Der Ausbau des öffentlichen Nahverkehrs ist natürlich sinnvoll. Aber wann kommt der? In Tschechien haben wir erlebt, wie solche Versprechen zehn Jahre dauern können.  
> **Mia:** Zwar sind die langfristigen Vorteile klar, aber die sofortige Belastung ist real. Ich finde, die Einführung der Citymaut muss von echten Alternativen begleitet werden — nicht nur dem Versprechen künftiger Busse.  
> **Lukas:** Da stimme ich zu. Die Entwicklung des Nahverkehrs und die Einführung der Citymaut sollten gleichzeitig passieren, nicht nacheinander.  
> **Pavel:** Nicht nur der Nahverkehr, sondern auch das Radwegenetz müsste ausgebaut werden. Mia, du fährst doch immer mit dem Fahrrad — bist du nicht automatisch dafür?  
> **Mia:** Ich bin Radfahrerin, ja. Aber ich versuche, fair zu denken. Die Verdrängung von Kleinunternehmern aus der Innenstadt wäre ein echter Schaden. Was wir brauchen, ist nicht nur das Verbot des Autofahrens, sondern die Förderung wirklicher Alternativen.  
> **Lukas:** Einverstanden. Der Rückgang der Emissionen ist das Ziel — der Weg dorthin muss sozial gerecht sein.  

### Dialog B: Das Op-Ed — Yusuf und Hannah (formell/akademisch)

*Scene: The El-Sayed family's living room in Charlottenburg. Evening. Yusuf
has drafted an op-ed (Gastkommentar) about integration policy for a Berlin
newspaper. He reads it aloud to Hannah, who gives feedback — particularly
pointing out where Verbalstil could be replaced by Nominalstil for a more
academic register.*

> **Yusuf:** Ich lese dir meinen ersten Entwurf vor. Hör zu und sag mir, wo es zu umgangssprachlich klingt. — „In Deutschland integrieren sich Migranten nicht, weil der Staat nicht ausreichend unterstützt."  
> **Hannah:** Der Satz stimmt inhaltlich, aber er klingt journalistisch, fast anklagend. In einem Gastkommentar würde ich schreiben: „Die mangelnde Integration von Migranten ist auf unzureichende staatliche Unterstützung zurückzuführen." Das ist Nominalstil — sachlicher, distanzierter.  
> **Yusuf:** Verstehe. Du meinst: statt „integrieren sich" besser „die Integration", statt „unterstützt" besser „die Unterstützung"?  
> **Hannah:** Genau. Wenn man einen Sachverhalt beschreibt, statt eine Meinung zu äußern, wirkt Nominalstil kompetenter. Zum Beispiel: statt „wir müssen die Umwelt schützen" — „der Schutz der Umwelt ist eine gesamtgesellschaftliche Aufgabe."  
> **Yusuf:** Gut. Hier der nächste Absatz: „Wenn Wohnungen teurer werden, können sich viele Menschen keine Unterkunft mehr leisten."  
> **Hannah:** Das ist Verbalstil. Auf Nominalstil: „Der Anstieg der Mietpreise führt bei vielen Menschen zur Wohnungsnot."  
> **Yusuf:** Der Anstieg statt „werden teurer" — ich sehe das Prinzip. Was ist mit: „Wenn man Energie spart, sinkt der CO₂-Ausstoß."?  
> **Hannah:** Durch das Energiesparen sinkt der CO₂-Ausstoß. Siehst du? Durch + Nomen ersetzt den wenn-Satz. Außerdem: „das Recyceln", „das Heizen", „das Autofahren" — Verben werden durch Großschreibung zu Nomen.  
> **Yusuf:** Wie beim Englischen Gerundium — aber im Deutschen ist es Neutrum, richtig?  
> **Hannah:** Richtig — alle nominalisierten Verben sind Neutrum: das Recyceln, das Heizen, das Fliegen, das Autofahren. Die mit -ung sind Femininum: die Nutzung, die Verwendung, die Verbesserung.  
> **Yusuf:** Und Adjektive? „Es ist wichtig, dass wir handeln."  
> **Hannah:** Die Wichtigkeit des Handelns ist unbestreitbar. Wichtig → die Wichtigkeit; möglich → die Möglichkeit; notwendig → die Notwendigkeit.  
> **Yusuf:** Hervorragend. Ich werde den ganzen Text überarbeiten. Die Qualität des Kommentars hängt von der Präzision der Sprache ab.  

---

## 2. Redemittel

### Meinungen äußern (expressing opinions)

| Deutsch | English |
|---|---|
| Meiner Meinung nach … | In my opinion … |
| Ich bin der Ansicht, dass … | I am of the opinion that … |
| Ich halte es für wichtig, dass … | I think it is important that … |
| Ich finde, dass … | I think / I find that … |
| Ich bin überzeugt, dass … | I am convinced that … |
| Ich bezweifle, dass … | I doubt that … |

### Strukturierte Argumentation (structured argument)

| Deutsch | English |
|---|---|
| Einerseits … andererseits … | On the one hand … on the other hand … |
| Zwar … aber … | Admittedly … but … |
| Es stimmt, dass … Trotzdem … | It is true that … Nevertheless … |
| Nicht nur … sondern auch … | Not only … but also … |
| Sowohl … als auch … | Both … and … |
| Weder … noch … | Neither … nor … |
| Ich teile diese Meinung (nicht). | I (don't) share this view. |
| Das leuchtet mir ein. | That makes sense to me. |

### Stellungnahme schreiben (writing a position paper)

| Deutsch | English |
|---|---|
| Zunächst möchte ich festhalten, dass … | First I would like to note that … |
| Dafür spricht, dass … | An argument in favour is that … |
| Dagegen spricht, dass … | An argument against is that … |
| Zusammenfassend lässt sich sagen, dass … | In summary, one can say that … |
| Abschließend bin ich der Meinung, dass … | In conclusion, I am of the opinion that … |

---

## 3. Wortschatz

### Umwelt und Klimawandel

| Nomen | Englisch |
|---|---|
| der Klimawandel, – | climate change |
| die Emission, -en | emission |
| das Treibhausgas, -e | greenhouse gas |
| die Luftverschmutzung, – | air pollution |
| der CO₂-Ausstoß, -ä-e | CO₂ output/emissions |
| das Recycling, – | recycling |
| die Nachhaltigkeit, – | sustainability |
| die Verkehrswende, – | shift in transport policy |
| das Elektroauto, -s | electric car |
| erneuerbare Energien (Pl.) | renewable energies |
| der Rückgang, -ä-e | decrease, decline |
| der Anstieg, -e | increase, rise |
| der Verbrauch, -ä-e | consumption, use |
| die Maßnahme, -n | measure, step |
| der Ausbau, -ten | expansion, development |
| die Einführung, -en | introduction, implementation |

### Gesellschaft und Sozialpolitik

| Nomen | Englisch |
|---|---|
| die Gleichstellung, – | equality (gender equality) |
| die Integration, -en | integration |
| die Wohnungsnot, – | housing shortage |
| die Gentrifizierung, -en | gentrification |
| der Mindestlohn, -ä-e | minimum wage |
| die Digitalisierung, – | digitalisation |
| soziale Gerechtigkeit (Sg.) | social justice |
| die Wohnungspolitik, -en | housing policy |
| die Förderung, -en | promotion, support, funding |
| die Belastung, -en | burden, strain |
| die Notwendigkeit, -en | necessity |

### Schlüsselverben

| Verb | Bedeutung |
|---|---|
| einführen ⚠️ separable | to introduce, to implement |
| ausbauen ⚠️ separable | to expand, to develop |
| reduzieren | to reduce |
| senken | to lower, to reduce |
| verringern | to diminish, to reduce |
| fördern | to promote, to fund, to support |
| begleiten | to accompany |
| überfällig (adj.) | overdue |
| überarbeiten ⚠️ inseparable | to revise |

---

## 4. Grammatik

### 4.1 Nominalisierung — turning verbs and adjectives into nouns

**What is Nominalisierung?**
Nominalisierung (nominalization) is the process of turning a verb or adjective
into a noun. It is extremely common in formal German — in newspaper texts,
academic writing, op-eds, and news reports — and is a key B1/B2 exam skill.
There are three main patterns.

---

**Pattern 1: Verb → Nomen with -ung (Femininum)**

Many verbs form nouns by adding **-ung** to the verb stem. These are always
**feminine** (die … -ung).

| Verb | Nomen (-ung) | Example |
|---|---|---|
| verbessern | die Verbesserung | die Verbesserung der Luftqualität |
| einführen | die Einführung | die Einführung der Citymaut |
| entwickeln | die Entwicklung | die Entwicklung des Nahverkehrs |
| nutzen / benutzen | die Nutzung / die Benutzung | die Nutzung erneuerbarer Energien |
| verwenden | die Verwendung | die Verwendung von Plastik |
| reduzieren | die Reduzierung | die Reduzierung der Emissionen |
| fördern | die Förderung | die Förderung des ÖPNV |
| belegen | die Belegung | — (less common) |
| erhöhen | die Erhöhung | die Erhöhung der Preise |
| anklagen | die Anklage | — (some -ung forms don't exist) |

> ⚠️ **Not every verb has a -ung form!** Check for the derived noun before
> using it. Common traps: *~~die Sinken~~* (doesn't exist) → use `der
> Rückgang` instead.

---

**Pattern 2: Verb → Nomen as Gerundium (Neutrum)**

Any German verb can be nominalized simply by capitalizing it. The resulting
noun is **always neuter** (das …).

| Verb | Gerundium | Example sentence |
|---|---|---|
| recyceln | das Recyceln | Das Recyceln von Plastik spart Ressourcen. |
| heizen | das Heizen | Das Heizen mit Gas ist teuer. |
| autofahren | das Autofahren | Das Autofahren in der Innenstadt wird teurer. |
| fliegen | das Fliegen | Das Fliegen erzeugt viel CO₂. |
| sparen | das Energiesparen | Durch das Energiesparen sinkt der Ausstoß. |

> 📌 **Merkasten — Gerundium (nominalized infinitive)**
>
> | Rule | Detail |
> |---|---|
> | **Always neuter** | das Recyceln, das Autofahren, das Heizen |
> | **Capitalized** | normal German noun rules apply |
> | **No article needed with `durch`** | Durch Recyceln (or: Durch das Recyceln) |
>
> Compare with English "-ing" forms: *"Recycling saves resources."*
> In German: *„Das Recyceln spart Ressourcen."* or
> *„Recyceln spart Ressourcen."* (without article, also correct)

---

**Pattern 3: Adjective → Nomen (Femininum, usually)**

Many adjectives form abstract nouns with **-keit** or **-heit** endings.
These are always **feminine** (die … -keit / -heit).

| Adjektiv | Nomen | Example |
|---|---|---|
| wichtig | die Wichtigkeit | Die Wichtigkeit dieses Themas ist klar. |
| möglich | die Möglichkeit | Es gibt keine andere Möglichkeit. |
| notwendig | die Notwendigkeit | Die Notwendigkeit von Maßnahmen ist unbestreitbar. |
| sicher | die Sicherheit | Die Sicherheit der Bürger hat Vorrang. |
| frei | die Freiheit | Die Freiheit der Presse ist wichtig. |
| ehrlich | die Ehrlichkeit | Ehrlichkeit ist eine Tugend. |
| schwierig | die Schwierigkeit | Die Schwierigkeit liegt im Detail. |

---

### 4.2 Verbalstil vs. Nominalstil

One of the most important B1+ skills is **switching between styles**:
**Verbalstil** (verb-heavy, conversational) and **Nominalstil** (noun-heavy,
formal/written).

| Verbalstil | Nominalstil |
|---|---|
| Wir müssen die Umwelt schützen. | Der Schutz der Umwelt ist notwendig. |
| Wenn man Energie spart, … | Durch das Energiesparen … |
| Die Mieten steigen. | Der Anstieg der Mietpreise … |
| Man integriert die Migranten nicht. | Die fehlende Integration der Migranten … |
| Man nutzt erneuerbare Energie. | Die Nutzung erneuerbarer Energien … |
| Die Luft verbessert sich. | Die Verbesserung der Luftqualität … |

**Key transformation tools:**

1. **Verb → -ung noun:** `verbessern → die Verbesserung` (then add Genitiv
   for the object: *die Verbesserung der Luft*)
2. **`wenn`-clause → `durch` + Gerundium:** `wenn man spart → durch das Sparen`
3. **`weil`-clause → `wegen` + Genitiv:** `weil die Preise steigen →
   wegen des Anstiegs der Preise` (very formal — see B1/07 for `wegen`)
4. **Predicate adjective → abstract noun:** `Das ist wichtig → Die Wichtigkeit`

> 📌 **Why does this matter for the exam?**
>
> The Goethe/telc B1 Schreiben task (Stellungnahme) rewards a **mix** of
> styles: Verbalstil for vivid opinions, Nominalstil for formal claims.
> Examiners notice when a candidate can switch between the two fluently.

---

### 4.3 Passiv — review in news and environment register

You learned Passiv in B1/04. Here is a focused review of the patterns most
common in newspaper language and environmental texts.

**Core pattern: werden + Partizip II**

| Tense | Formula | Example |
|---|---|---|
| Präsens Passiv | wird/werden + Part. II | Jährlich werden Millionen Tonnen CO₂ ausgestoßen. |
| Präteritum Passiv | wurde/wurden + Part. II | Neue Gesetze wurden eingeführt. |
| Perfekt Passiv | ist/sind + Part. II + worden | Die Maßnahmen sind beschlossen worden. |
| Passiv mit Modal | modal + Part. II + werden (Inf.) | Die Ziele müssen erreicht werden. |

**Common environmental Passiv phrases:**

> *Es werden jährlich Millionen Tonnen CO₂ ausgestoßen.*
> (Every year, millions of tonnes of CO₂ are emitted.)
>
> *Neue Maßnahmen sollen eingeführt werden.*
> (New measures are to be introduced.)
>
> *Erneuerbare Energien werden zunehmend genutzt.*
> (Renewable energies are increasingly being used.)
>
> *Das Recycling muss verbessert werden.*
> (Recycling must be improved.)
>
> *Die Emissionen wurden in den letzten zehn Jahren deutlich reduziert.*
> (Emissions have been significantly reduced over the last ten years.)

> ⚠️ **News language tip:** In headlines and news reports, Passiv often
> appears **without the agent** (`von + Dativ`). The focus is on the action
> or result, not who performed it. This is the most common use of Passiv at
> B1 level.

---

## 5. Lesetext — Gastkommentar: Die Verkehrswende braucht soziale Gerechtigkeit

*Lies den Text zweimal: einmal schnell, einmal langsam. Er wird in Block C
verwendet.*

---

*Von Dr. Yusuf El-Sayed, erschienen in der Berliner Stadtzeitung*

**Die Einführung der Citymaut ist überfällig — aber sie reicht nicht aus.**

Die Reduzierung des städtischen Autoverkehrs wird in vielen europäischen
Metropolen bereits praktiziert. Die Nutzung von Innenstadtmaut-Systemen ist
kein Experiment mehr: In London, Stockholm und Oslo wurden bereits messbare
Erfolge erzielt. Jährlich werden in diesen Städten deutlich weniger
Treibhausgase ausgestoßen als vor der Einführung dieser Maßnahmen.

Nun plant Berlin dasselbe. Die Wichtigkeit eines solchen Schritts ist kaum zu
überschätzen: Die Luftverschmutzung in Berlins Innenstadt überschreitet
regelmäßig EU-Grenzwerte. Erneuerbare Energien allein reichen nicht aus, wenn
gleichzeitig Millionen von Dieselfahrzeugen die Straßen der Hauptstadt
verstopfen. Der Rückgang der Emissionen kann nur durch eine Kombination von
Maßnahmen erreicht werden.

Einerseits ist die Verbesserung der Luftqualität ein legitimes Ziel der
Stadtpolitik. Andererseits darf die Belastung nicht einseitig auf kleine
Betriebe und einkommensschwache Haushalte verteilt werden. Die Förderung des
öffentlichen Nahverkehrs muss parallel zur Einführung der Maut erfolgen —
nicht Jahre danach. Zwar sind die ökologischen Vorteile klar, aber die soziale
Gerechtigkeit darf nicht vergessen werden.

Nicht nur die Verkehrswende, sondern auch der Ausbau des Radwegenetzes und
die Förderung des Carsharing müssen Teil eines umfassenden Konzepts sein. Die
Notwendigkeit, alle Gesellschaftsgruppen einzubeziehen, ist unbestreitbar.
Zusammenfassend lässt sich sagen: Die Einführung der Citymaut ist ein
richtiger Schritt. Aber die Entwicklung eines sozial gerechten Gesamtpakets
ist ebenso wichtig — wenn nicht wichtiger.

---

## 6. Hörtext — Anna und Bruno diskutieren: Plastikverbot im Supermarkt

*Hör zu und mach Übung H3. Öffne das Transkript erst nach dem Hören!*

<details>
<summary>📄 Transkript (erst nach dem Hören öffnen!)</summary>

> Anna und Bruno diskutieren im Supermarkt über das neue Plastikverbot. Anna findet, die Verwendung von Plastikverpackungen hätte schon lange verboten werden müssen. Bruno versteht das Argument, aber er findet, dass die Belastung für Verbraucher größer geworden ist, weil Alternativen teurer sind. Anna antwortet, der Rückgang des Plastikmülls zeige, dass die Maßnahme wirkt. Bruno gibt zu, dass die Nutzung von Mehrwegverpackungen sich allmählich durchsetzt. Beide sind sich einig: Die Notwendigkeit des Recycelns ist unbestreitbar, aber die Förderung von Alternativen muss weiter verstärkt werden.

</details>

---

## 7. Lernstrategie — Nominalstil erkennen und üben

**How to spot Nominalstil in German texts:**

Look for these signals in a newspaper or academic text:
- Long noun phrases: *die Einführung der Citymaut* (3+ words)
- `-ung` endings everywhere: Einführung, Verbesserung, Entwicklung
- `durch` + noun replacing a `wenn/weil`-clause
- Genitive chains: *der Rückgang der Emissionen der Fahrzeuge*
- Verbs like `erfolgen`, `stattfinden`, `durchführen` — typical "Nominalstil
  support verbs" that let the real content go into the noun.

**The 3-step transformation drill:**

Pick any German news headline. Find every verb. Ask:
1. Does the verb have a `-ung` form? → Use it.
2. Is it a separable verb? → Nominalize the whole compound: `ausbauen → der Ausbau`.
3. Is it in a `wenn`/`weil` clause? → Replace with `durch/wegen + Nomen`.

**Example:**
> *Headline:* „Berlin will Fahrräder fördern und Autos einschränken."
> *Nominalstil:* „Die Förderung von Fahrrädern und die Einschränkung des
> Autoverkehrs sind Berlins Ziele."

This daily habit — 5 minutes of text transformation — builds the Nominalstil
reflex that examiners reward at B1+.

➡️ Weiter zu den Übungen: siehe `exercises.md` (generiert aus `exercises.yml`).
