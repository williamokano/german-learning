...
> talk about how things are made and what rules apply, read and write news-style
> sentences, and use the passive with modal verbs to express obligations and
> possibilities.
>
> **Grammar:** Passiv Präsens (`wird … gemacht`) · Passiv Präteritum
> (`wurde … gemacht`) · Passiv Perfekt (`ist … gemacht worden`) · Passiv with
> modals (`muss … gemacht werden`, `kann … gebaut werden`) · agent phrase
> `von + Dativ`.
>
> **Builds on:** B1/01 (Präteritum — Passiv Präteritum uses `wurde`) ·
> A2/04 (`wenn`-clauses — Passiv + `wenn`) · B1/02 (connectors `deshalb`,
> `obwohl` — used to chain passive sentences).
...
> **Lukas:** Ehrlich gesagt nicht. Viele Routineaufgaben werden heute von Algorithmen erledigt — das stimmt. Früher wurden solche Aufgaben von Menschen gemacht. Aber dadurch entstehen ja auch neue Jobs.  
> **Bruno:** Ich sehe das ähnlich. Bei uns in der IT-Beratung werden gerade viele Reports automatisch generiert. Das wurde früher stundenlang von Assistenten gemacht.  
> **Anna:** Und was passiert mit den Assistenten?  
> **Lukas:** Sie werden für komplexere Aufgaben eingesetzt. Zumindest in guten Unternehmen. Das System muss natürlich gut eingeführt werden — das kann nicht einfach von heute auf morgen gemacht werden.  
> **Bruno:** Genau. Bei uns ist ein neues Tool eingeführt worden, und am Anfang war das Chaos. Nichts wurde erklärt, die Mitarbeiter wurden nicht vorbereitet.  
> **Anna:** Das ist typisch. Technologie wird oft gekauft, aber die Menschen werden vergessen.  
...
> **Frau Weber:** Guten Morgen, Frau Kowalski. Ich wollte Sie kurz über den neuen Reporting-Prozess informieren.  
> **Anna:** Guten Morgen, Frau Weber. Ja, gerne.  
> **Frau Weber:** Ab nächsten Monat werden alle monatlichen Berichte automatisch erstellt. Die Daten werden aus unserem CRM-System gezogen und in einer Vorlage zusammengestellt.  
> **Anna:** Das klingt effizient. Muss ich dann trotzdem noch etwas prüfen?  
> **Frau Weber:** Ja. Alle Zahlen müssen vorher von Ihnen überprüft werden, bevor der Bericht freigegeben wird. Das ist sehr wichtig — Fehler dürfen nicht ignoriert werden.  
> **Anna:** Verstehe. Und wann muss der Bericht fertiggestellt werden?  
> **Frau Weber:** Er muss bis zum 5. jedes Monats abgeschlossen und vom Abteilungsleiter genehmigt werden.  
> **Anna:** Und wenn ein Fehler gefunden wird — was soll dann gemacht werden?  
> **Frau Weber:** Der Fehler muss sofort gemeldet werden. Dann wird der Bericht korrigiert und neu abgeschickt. Der Prozess ist jetzt so gestaltet worden, dass Korrekturen schnell vorgenommen werden können.  
> **Anna:** Gut. Und wurden die anderen Abteilungen auch schon informiert?  
> **Frau Weber:** Ja, alle Teams sind bereits benachrichtigt worden. Die Schulungen wurden letzte Woche durchgeführt. Haben Sie noch Fragen?  
> **Anna:** Nein, ich denke, das ist alles klar. Danke, Frau Weber.  
...
| German | English |
|---|---|
| Das darf nicht ignoriert werden. | That must not be ignored. |
| Der Fehler muss gemeldet werden. | The error must be reported. |
| Alle Daten müssen gesichert werden. | All data must be backed up. |
| Das kann sofort korrigiert werden. | That can be corrected immediately. |
| Es soll bis Freitag abgeschlossen werden. | It should be completed by Friday. |

### Nachrichten und Ankündigungen (news language)

| German | English |
|---|---|
| Es wurde bekannt gegeben, dass … | It was announced that … |
| Das Gebäude wurde eröffnet. | The building was opened. |
| Die Regelung wurde geändert. | The regulation was changed. |
| Das Ergebnis ist veröffentlicht worden. | The result has been published. |
| Es ist bestätigt worden, dass … | It has been confirmed that … |
...
| Nomen | Englisch |
|---|---|
| die Herstellung, -en | production, manufacturing |
| der Prozess, -e | process |
| die Produktion, -en | production |
| das Verfahren, – | procedure, process |
| die Anlage, -n | plant, facility |
| der Rohstoff, -e | raw material |
| das Bauteil, -e | component, part |
| die Fabrik, -en | factory |
| die Qualität, -en | quality |
| die Kontrolle, -n | check, inspection |
| die Genehmigung, -en | approval, permit |
| der Bericht, -e | report |
| die Vorlage, -n | template; submission |
| der Fehler, – | error, mistake |
| die Schulung, -en | training (course) |

| Verb | Englisch |
|---|---|
| herstellen | to manufacture, produce |
| verarbeiten | to process |
| bauen ⚠️ (Partiz. II: gebaut) | to build |
| prüfen | to check, test |
| genehmigen | to approve |
| zusammenstellen | to compile, assemble |
| überprüfen | to verify, review |
| korrigieren | to correct |
| einführen | to introduce, implement |
| freigeben ⚠️ (sep., Partiz. II: freigegeben) | to release, approve |
| vornehmen ⚠️ (sep., Partiz. II: vorgenommen) | to carry out, make (changes) |

⚠️ **Partizip II reminder:** `herstellen` → `hergestellt`; `einführen` → `eingeführt`;
`freigeben` → `freigegeben`; `vornehmen` → `vorgenommen`.
...
| bekannt geben ⚠️ (sep., Partiz. II: bekannt gegeben) | to announce |
| ankündigen | to announce, announce in advance |
| veröffentlichen | to publish |
| bestätigen | to confirm |
| eröffnen | to open (officially) |
| beschließen ⚠️ (unregelmäßig, Partiz. II: beschlossen) | to decide, resolve |

| Nomen | Englisch |
|---|---|
| die Meldung, -en | report, news item |
| die Ankündigung, -en | announcement |
| die Veröffentlichung, -en | publication |
...
| er/sie/es | wird | gefragt | Er wird gefragt. |
...
| er/sie/es | wurde | gefragt | Er wurde gefragt. |
| wir | wurden | gefragt | Wir wurden gefragt. |
...
Ländern importiert und in spezialisierten Anlagen verarbeitet. Früher wurden
diese Materialien fast ausschließlich von Benzinmotoren-Herstellern genutzt —
heute werden sie für Hochleistungsbatterien verwendet.

In der Fabrik werden zunächst die Karosserieteile aus Stahl und Aluminium
gefertigt. Sie werden von Robotern zusammengesetzt und anschließend von
spezialisierten Technikern überprüft. Jedes Bauteil muss sorgfältig geprüft
werden, bevor es weiterverarbeitet wird. Fehler dürfen in diesem Stadium nicht
übersehen werden.

Die Batterie ist das Herzstück des Elektroautos. Sie wird separat gebaut und
dann in das Fahrzeug eingebaut. Seit 2020 ist die Batteriekapazität erheblich
gesteigert worden — durch neue Materialien und verbesserte Produktionsverfahren.
Auch die Ladezeit ist deutlich verkürzt worden.

Am Ende der Produktionslinie wird jedes Auto einem strengen Test unterzogen.
Erst wenn alle Prüfungen bestanden worden sind, darf das Fahrzeug das Werk
verlassen. Das fertige Auto wird dann in Händlernetzwerke ausgeliefert und
...
> ist heute Morgen offiziell eröffnet worden. Das Gebäude wurde 2021 geplant und
> vor drei Jahren von einem internationalen Architekturbüro entworfen. Die
> Eröffnung wurde ursprünglich für letztes Jahr geplant, musste aber wegen
> Lieferproblemen verschoben werden. Ab sofort können Eintrittskarten online
> gebucht werden. Außerdem wurde heute bekannt gegeben, dass die Fahrradwege im
> Bezirk Mitte bis Ende des Jahres ausgebaut werden sollen.
