# B1 · Lektion 8 — Verben mit Präpositionen II (Verbs with Fixed Prepositions II)

> **You will learn to:** talk about feelings, opinions, and reactions using verbs
> with fixed prepositions; replace prepositional phrases with Pronominaladverbien
> (`darauf`, `daran`, `darüber`, `damit`, `davon`, `davor`); ask questions about
> things with `wo(r)-` compounds (`worüber?`, `worauf?`, `woran?`).
>
> **Grammar:** Full B1 set of verbs with fixed prepositions (an, auf, für, mit,
> über, um, von, vor, zu) · Pronominaladverbien: `da(r)- + Präposition` replaces
> `Präp + das/es` for things/ideas; `wo(r)- + Präposition` for questions about
> things · Anticipatory `da(r)-`: `Ich freue mich darauf, dass …` / `darauf, … zu` ·
> Person rule: `Auf wen?` (person) vs. `Worauf?` (thing)
>
> **Builds on:** A2/06 (verbs with prepositions I — basic set: warten auf,
> sich freuen über, sprechen über, sich ärgern über, sich interessieren für,
> träumen von, sich freuen auf) · B1/05 (Relativsätze — `worüber` parallels
> relative `über das`) · B1/06 (Infinitiv mit zu — anticipatory `da(r)-` +
> dass/zu appears here)

---

## 1. Dialoge

### Dialog A: Nach dem Film — Mias Wohnung (informal, 3 Personen)

*Scene: Mia's apartment in Kreuzberg, late evening. Mia, Pavel, and Tomáš have just
finished watching a film together (a Czech drama with German subtitles). They're
discussing their reactions over wine.*

> **Mia:** So — was haltet ihr davon? Ich interessiere mich eigentlich nicht so sehr für Dramen, aber dieser Film hat mich wirklich beschäftigt.  
> **Tomáš:** Ich auch! Ich habe schon den ganzen Abend daran gedacht. Die Hauptfigur — man kann sich so gut mit ihr identifizieren. Ich träume manchmal davon, einfach alles hinzuschmeißen und neu anzufangen.  
> **Pavel:** Ich habe mich ehrlich gesagt ein bisschen darüber geärgert, dass das Ende so offen war. Ich warte immer auf einen klaren Abschluss.  
> **Mia:** Ich weiß, was du meinst. Aber ich finde, genau darauf kommt es an — dass man selbst entscheidet, wie man es deutet.  
> **Tomáš:** Stimmt. Ich freue mich übrigens darüber, dass wir das zusammen geschaut haben. Ich dachte erst, Pavel interessiert sich gar nicht dafür.  
> **Pavel:** Doch, ich interessiere mich sehr dafür. Ich habe mich nur nicht drum gekümmert, einen Film vorzuschlagen. Das ist meine Schwäche — ich kümmere mich nie darum.  
> **Mia:** Du musst dich nicht dafür entschuldigen! Ich habe mich doch gefreut, ihn auszusuchen. Woran denkst du jetzt am meisten nach dem Film?  
> **Tomáš:** Ich denke daran, wie anders das Leben in Prag sein könnte. Der Film handelt ja davon, wie man sich von einer Stadt löst. Ich kann mich gut darauf einlassen.  
> **Pavel:** Ich denke vor allem an die letzte Szene. Ich warne euch: Ich verlasse mich darauf, dass es eine Fortsetzung gibt!  
> **Mia:** Ich freue mich schon darauf! Aber jetzt: Wein nach? Ich bestehe darauf!  

### Dialog B: Vorstellungsgespräch — formell (formal, 2 Personen)

*Scene: A Berlin startup. Anna is being interviewed by Frau Weber for a project
coordinator role.*

> **Frau Weber:** Frau Kowalski, herzlich willkommen. Ich freue mich darüber, dass Sie sich für diese Stelle beworben haben. Können Sie uns kurz erzählen, wofür Sie sich besonders interessieren?  
> **Anna:** Vielen Dank, Frau Weber. Ich interessiere mich sehr für digitales Projektmanagement und für internationale Teamarbeit. Ich beschäftige mich schon seit zwei Jahren damit.  
> **Frau Weber:** Gut. Worauf verlassen Sie sich, wenn Sie unter Druck arbeiten?  
> **Anna:** Ich verlasse mich darauf, klare Strukturen zu haben. Ich achte immer darauf, Prioritäten früh zu setzen und gut mit meinem Team zu kommunizieren.  
> **Frau Weber:** Und worum kümmern Sie sich in Ihrer jetzigen Stelle hauptsächlich?  
> **Anna:** Ich kümmere mich darum, dass alle Projektschritte dokumentiert werden, und ich sorge dafür, dass die Deadlines eingehalten werden.  
> **Frau Weber:** Diese Stelle besteht aus mehreren Teilbereichen. Wovon hängt Ihrer Meinung nach der Erfolg eines Projekts ab?  
> **Anna:** Der Erfolg hängt vor allem davon ab, wie gut sich die Teammitglieder aufeinander verlassen können. Außerdem kommt es darauf an, dass man flexibel reagiert, wenn sich die Anforderungen ändern.  
> **Frau Weber:** Sie haben sich also auch für eine frühere Stelle in diesem Bereich beworben?  
> **Anna:** Ja, ich habe mich letztes Jahr um eine ähnliche Position beworben, aber ich bin leider nicht dazu gekommen, das Gespräch zu führen — ich musste mich damals um einen Umzug kümmern.  
> **Frau Weber:** Ich bin überzeugt davon, dass Ihre Erfahrung hier gut passt. Wir werden uns bald bei Ihnen melden.  
> **Anna:** Danke. Ich freue mich darauf.  

---

## 2. Redemittel

### Gefühle und Meinungen ausdrücken (expressing feelings and opinions)

| German | English |
|---|---|
| Ich freue mich darüber, dass … | I'm happy/glad that … |
| Ich freue mich darauf, … zu … | I'm looking forward to … |
| Ich ärgere mich darüber, dass … | I'm annoyed that … |
| Ich interessiere mich dafür. | I'm interested in it. |
| Ich denke daran, … zu … | I'm thinking of … |
| Ich träume davon, … zu … | I dream of … |
| Ich freue mich darauf! | I'm looking forward to it! |

### Reaktionen und Einstellungen (reactions and attitudes)

| German | English |
|---|---|
| Ich verlasse mich darauf, dass … | I rely on (the fact that) … |
| Ich achte darauf, dass … | I make sure that …, I pay attention to … |
| Es kommt darauf an, dass … | It depends on (the fact that) … |
| Es handelt sich darum, dass … | It's about the fact that … |
| Ich bin davon überzeugt, dass … | I'm convinced that … |
| Ich warne davor, … zu … | I warn (you) against … |
| Ich bestehe darauf! | I insist on it! |

### Formelle Ausdrücke (formal expressions)

| German | English |
|---|---|
| Ich bewerbe mich darum / dafür. | I am applying for it. |
| Ich kümmere mich darum. | I'll take care of it. |
| Ich sorge dafür, dass … | I ensure that … |
| Der Erfolg hängt davon ab, dass … | The success depends on … |
| Die Stelle besteht daraus, … zu … | The position consists of … |
| Ich bitte Sie darum, … zu … | I kindly ask you to … |

---

## 3. Wortschatz

### Verben mit festen Präpositionen — Vollständige B1-Liste

| Verb | Präposition | Kasus | Beispiel |
|---|---|---|---|
| denken | an | Akk. | Ich denke an den Film. |
| sich erinnern | an | Akk. | Erinnerst du dich daran? |
| glauben | an | Akk. | Ich glaube an die Zukunft. |
| teilnehmen | an | Dat. | Er nimmt an dem Kurs teil. |
| arbeiten | an | Dat. | Sie arbeitet an dem Projekt. |
| warten | auf | Akk. | Wir warten auf eine Antwort. |
| sich freuen | auf | Akk. | Ich freue mich auf das Wochenende. |
| sich verlassen | auf | Akk. | Ich verlasse mich auf dich. |
| reagieren | auf | Akk. | Wie reagierst du darauf? |
| achten | auf | Akk. | Ich achte auf die Qualität. |
| bestehen | auf | Dat. | Er besteht auf seinem Recht. |
| sich interessieren | für | Akk. | Sie interessiert sich für Kunst. |
| sich bedanken | für | Akk. | Ich bedanke mich dafür. |
| sich entscheiden | für | Akk. | Sie entscheidet sich dafür. |
| sorgen | für | Akk. | Er sorgt für die Familie. |
| sich beschäftigen | mit | Dat. | Ich beschäftige mich damit. |
| rechnen | mit | Dat. | Wir rechnen damit. |
| sprechen | mit | Dat. | Ich spreche mit ihr. |
| sprechen | über | Akk. | Wir sprechen über das Thema. |
| sich freuen | über | Akk. | Sie freut sich darüber. |
| sich ärgern | über | Akk. | Er ärgert sich darüber. |
| nachdenken | über | Akk. | Ich denke darüber nach. |
| sich beschweren | über | Akk. | Er beschwert sich darüber. |
| sich kümmern | um | Akk. | Sie kümmert sich darum. |
| sich bewerben | um / für | Akk. | Er bewirbt sich darum / dafür. |
| bitten | um | Akk. | Ich bitte Sie darum. |
| träumen | von | Dat. | Ich träume davon. |
| abhängen | von | Dat. | Es hängt davon ab. |
| überzeugt sein | von | Dat. | Ich bin davon überzeugt. |
| sich verabschieden | von | Dat. | Sie verabschiedet sich davon. |
| sich fürchten | vor | Dat. | Er fürchtet sich davor. |
| warnen | vor | Dat. | Ich warne dich davor. |
| Angst haben | vor | Dat. | Sie hat Angst davor. |
| gehören | zu | Dat. | Das gehört dazu. |
| beitragen | zu | Dat. | Es trägt dazu bei. |
| führen | zu | Dat. | Das führt dazu. |

### Gefühle und Reaktionen (feelings and reactions)

| Nomen | Englisch |
|---|---|
| die Meinung, -en | opinion |
| die Reaktion, -en | reaction |
| das Gefühl, -e | feeling |
| die Überzeugung, -en | conviction, belief |
| die Einstellung, -en | attitude |
| die Begeisterung, – | enthusiasm |
| die Enttäuschung, -en | disappointment |
| die Neugier, – | curiosity |
| die Bedenken (Pl.) | concerns, reservations |
| der Eindruck, -ü-e (die Eindrücke) | impression |

### Interview- und Bewerbungswortschatz

| Ausdruck | Bedeutung |
|---|---|
| die Stelle, -n | position, job |
| der Teilbereich, -e | sub-area, component area |
| die Anforderung, -en | requirement |
| die Deadline, -s | deadline |
| die Struktur, -en | structure |
| die Priorität, -en | priority |
| die Dokumentation, -en | documentation |
| die Erfahrung, -en | experience |
| der Erfolg, -e | success |
| die Zusammenarbeit, – | collaboration |

---

## 4. Grammatik

### 4.1 Verben mit festen Präpositionen — warum "fest"?

Unlike in English, many German verbs **require a specific preposition** that cannot
be changed. You cannot substitute another preposition — the combination is fixed
(hence "fest"). Compare:

- *Ich warte **auf** den Bus.* (always `auf`, never `nach` or `für`)
- *Ich träume **von** dem Urlaub.* (always `von`, never `über`)
- *Ich ärgere mich **über** das Wetter.* (always `über`, never `wegen`)

The preposition changes the **case** of the noun it governs. The case is also
fixed — it does not change depending on position:

| Präposition | Kasus bei Verb | Example |
|---|---|---|
| `an` | Akk. (denken an, sich erinnern an) | Ich denke **an den** Film. |
| `an` | Dat. (teilnehmen an, arbeiten an) | Er arbeitet **an dem** Projekt. |
| `auf` | Akk. (warten auf, sich freuen auf…) | Wir warten **auf eine** Antwort. |
| `auf` | Dat. (bestehen auf) | Er besteht **auf seinem** Recht. |
| `für` | Akk. (sich interessieren für, sorgen für…) | Sie sorgt **für die** Familie. |
| `mit` | Dat. (sich beschäftigen mit, sprechen mit…) | Ich beschäftige mich **damit**. |
| `über` | Akk. (sprechen über, sich ärgern über…) | Wir sprechen **über das** Thema. |
| `um` | Akk. (sich kümmern um, sich bewerben um…) | Sie kümmert sich **um das** Problem. |
| `von` | Dat. (träumen von, abhängen von…) | Das hängt **von der** Situation ab. |
| `vor` | Dat. (Angst haben vor, sich fürchten vor…) | Er fürchtet sich **vor dem** Hund. |
| `zu` | Dat. (gehören zu, beitragen zu…) | Das gehört **zu dem** Plan. |

> ⚠️ **Two prepositions, one verb: `sich freuen`**
>
> | Form | Meaning |
> |---|---|
> | Ich freue mich **auf** etwas (Akk.) | I look forward to sth. (future event) |
> | Ich freue mich **über** etwas (Akk.) | I am glad about sth. (present/past fact) |
>
> *Ich freue mich **auf** das Konzert.* (still in the future)
> *Ich freue mich **über** dein Geschenk.* (I already have it / it happened)

---

### 4.2 Pronominaladverbien — `da(r)- + Präposition`

When you want to replace a **prepositional phrase** that refers to a **thing or
idea** (not a person), German uses a **Pronominaladverb** instead of
`Präposition + Pronomen`:

| Normal pronoun (person) | Pronominaladverb (thing/idea) |
|---|---|
| Ich denke **an ihn**. (person) | Ich denke **daran**. (thing/idea) |
| Ich warte **auf sie**. (person) | Ich warte **darauf**. (thing/idea) |
| Ich spreche **mit ihm**. (person) | Ich spreche **damit**. (thing/idea) |
| Ich träume **von ihr**. (person) | Ich träume **davon**. (thing/idea) |

**Formation rule:**
- Preposition starts with a **consonant** → `da + Präposition`: `dafür`, `damit`, `davon`, `davor`, `dazu`
- Preposition starts with a **vowel** → `dar + Präposition`: `daran`, `darauf`, `darüber`, `darum`

| Präposition | Pronominaladverb | Example |
|---|---|---|
| an | **daran** | Ich denke **daran**. |
| auf | **darauf** | Ich warte **darauf**. |
| über | **darüber** | Ich freue mich **darüber**. |
| für | **dafür** | Ich interessiere mich **dafür**. |
| mit | **damit** | Ich beschäftige mich **damit**. |
| von | **davon** | Ich träume **davon**. |
| vor | **davor** | Ich warne dich **davor**. |
| um | **darum** | Ich kümmere mich **darum**. |
| zu | **dazu** | Das gehört **dazu**. |

> 📌 **Merkasten — `da(r)-` vs. `Präp + Pronomen`**
>
> | Rule | Example |
> |---|---|
> | **Thing/idea → Pronominaladverb** | Ich denke **daran** (an den Film). |
> | **Person → Präp + Pronomen** | Ich denke **an ihn** (an Pavel). |
> | **Consonant: `da-`** | `da` + `für` = **dafür** |
> | **Vowel: `dar-`** | `dar` + `auf` = **darauf** |

⚠️ **Common trap:** German learners often say `*Ich denke daran ihn*` — wrong!
For persons: *Ich denke an ihn* (no `da-` form).

---

### 4.3 Fragen mit `wo(r)- + Präposition`

To ask **what** thing or idea someone is referring to, use `wo(r)- + Präposition`:

| Question | Answer |
|---|---|
| **Worüber** redest du? | Ich rede **darüber** / über den Film. |
| **Worauf** wartest du? | Ich warte **darauf** / auf den Bus. |
| **Woran** denkst du? | Ich denke **daran** / an den Urlaub. |
| **Womit** beschäftigst du dich? | Ich beschäftige mich **damit** / mit dem Projekt. |
| **Wovon** träumst du? | Ich träume **davon** / von einer Reise. |
| **Wofür** interessierst du dich? | Ich interessiere mich **dafür** / für Kunst. |
| **Wovor** hast du Angst? | Ich habe Angst **davor** / vor dem Prüfungsstress. |
| **Worum** geht es? | Es geht **darum** / um die Entscheidung. |

**Formation rule** — same as `da(r)-`:
- Consonant → `wo + Präp`: `wofür`, `womit`, `wovon`, `wovor`, `wozu`
- Vowel → `wor + Präp`: `woran`, `worauf`, `worüber`, `worum`

> ⚠️ **Person or thing?**
>
> | Person | Thing/Idea |
> |---|---|
> | **Auf wen** wartest du? → Auf Pavel. | **Worauf** wartest du? → Auf den Bus. |
> | **Mit wem** sprichst du? → Mit Mia. | **Womit** beschäftigst du dich? → Mit dem Bericht. |
> | **An wen** denkst du? → An meine Mutter. | **Woran** denkst du? → An den Urlaub. |

---

### 4.4 Anticipatory `da(r)-` — Korrelat

When the prepositional verb is followed by a **`dass`-clause** or a **`zu`-Infinitive**,
the Pronominaladverb appears **before the clause** as an **anticipatory marker**
(Korrelat). This is very common in German and sounds natural:

**With `dass`-clause:**

> *Ich freue mich **darüber**, dass du gekommen bist.*
> (I'm happy that you came.)
>
> *Ich bin **davon** überzeugt, dass sie die Beste ist.*
> (I'm convinced that she is the best.)
>
> *Es kommt **darauf** an, dass du pünktlich bist.*
> (It depends on your being punctual.)

**With `zu`-Infinitive (same subject):**

> *Ich freue mich **darauf**, dich zu sehen.*
> (I'm looking forward to seeing you.)
>
> *Ich achte **darauf**, Prioritäten früh zu setzen.*
> (I make sure to set priorities early.)
>
> *Ich warne dich **davor**, das zu tun.*
> (I warn you against doing that.)

> 📌 **Merkasten — Anticipatory `da(r)-`**
>
> | Structure | Example |
> |---|---|
> | Verb + `da(r)-` + `, dass …` | Ich freue mich **darüber**, **dass** du kommst. |
> | Verb + `da(r)-` + `, … zu …` | Ich freue mich **darauf**, **zu** reisen. |
> | The `da(r)-` cannot be omitted | *~~Ich freue mich, dass du kommst~~* → ❌ needs `darüber` |
>
> ⚠️ Note: some verbs optionally include or omit the Korrelat. With `sich freuen
> über` + `dass`, the `darüber` is strongly preferred. With `hoffen`, the Korrelat
> is usually omitted: *Ich hoffe, dass du kommst.* (no `dafür`).

---

## 5. Lesetext — Interview mit Anna: Mein erstes Jahr als Projektkoordinatorin

*Lies den Text zweimal: einmal schnell, einmal langsam. Er wird in Block C verwendet.*

Die Berliner Startup-Zeitschrift *Gründerzeit* hat Anna Kowalski interviewt, die
seit einem Jahr als Junior-Projektkoordinatorin bei einem Tech-Startup in Mitte
arbeitet.

---

**Gründerzeit:** Frau Kowalski, worüber haben Sie sich in Ihrem ersten Jahr am
meisten gefreut?

**Anna:** Ich habe mich darüber gefreut, dass mein Team mir von Anfang an vertraut
hat. Ich komme aus Russland, und ich war zunächst davon überzeugt, dass die
Sprache eine große Hürde sein würde. Aber es hat gut geklappt. Ich freue mich
jeden Tag darauf, ins Büro zu kommen.

**Gründerzeit:** Worum kümmern Sie sich hauptsächlich?

**Anna:** Ich kümmere mich darum, dass alle Projektschritte klar dokumentiert sind.
Außerdem sorge ich dafür, dass die Kommunikation zwischen den Teams funktioniert.
Es kommt oft darauf an, schnell auf neue Anforderungen zu reagieren. Das finde
ich spannend.

**Gründerzeit:** Wovon hängt Ihrer Meinung nach der Erfolg eines Projekts ab?

**Anna:** Er hängt davon ab, wie sehr sich die Menschen aufeinander verlassen können.
Ich achte immer darauf, eine vertrauensvolle Atmosphäre zu schaffen. Ich glaube
fest daran, dass Transparenz entscheidend ist.

**Gründerzeit:** Was empfehlen Sie anderen Berufseinsteigern?

**Anna:** Ich warne davor, sich zu sehr unter Druck zu setzen. Gleichzeitig rate
ich dazu, Fragen zu stellen — auch wenn man Angst davor hat, unwissend zu wirken.
Ich interessiere mich sehr für offene Lernkulturen in Unternehmen, und ich bin
davon überzeugt, dass sie den Unterschied machen.

---

## 6. Hörtext — Pavel spricht mit Mia über seine Arbeit

*Hör zu und mach Übung H3. Öffne das Transkript erst nach dem Hören!*

<details>
<summary>📄 Transkript (erst nach dem Hören öffnen!)</summary>

> Pavel ruft Mia an, um ihr von einem neuen Kurs zu erzählen. Er beschäftigt sich schon seit Wochen damit, die richtige Ausbildung zu finden. Er freut sich sehr darauf, endlich anzufangen. Er interessiert sich besonders dafür, wie man Röstprofile entwickelt. Allerdings warnt er Mia davor, zu erwarten, dass er jetzt weniger Zeit für Freunde hat — er achtet darauf, dass Arbeit und Freizeit in Balance bleiben. Mia freut sich darüber, dass Pavel so begeistert ist.

</details>

---

## 7. Lernstrategie — Die da(r)-Kette

The fastest way to make Pronominaladverbien automatic is **chunking the verb with
its `da(r)-` form**. Don't learn verbs in isolation — learn them as fixed pairs:

| Verb (+ Präp) | `da(r)-` Form | Chunk |
|---|---|---|
| sich freuen auf | darauf | *Ich freue mich darauf.* |
| sich ärgern über | darüber | *Ich ärgere mich darüber.* |
| denken an | daran | *Ich denke daran.* |
| sich kümmern um | darum | *Ich kümmere mich darum.* |
| träumen von | davon | *Ich träume davon.* |
| achten auf | darauf | *Ich achte darauf.* |
| überzeugt sein von | davon | *Ich bin davon überzeugt.* |

**Method:** Make a flashcard for each pair. Front: the English meaning + a blank.
Back: the full German chunk. Drill 5 minutes a day. By day 7, the `da(r)-` will
feel as natural as the verb itself.

**Extension — write a chain:** Take any topic (a film you saw, a plan you have)
and write 4–5 sentences using different `da(r)-` forms:
> *Ich habe an den Film gedacht. Ich freue mich darüber, dass wir ihn zusammen
> geschaut haben. Ich denke noch immer daran. Ich träume davon, dass es eine
> Fortsetzung gibt.*

➡️ Weiter zu den Übungen: siehe `exercises.md` (generiert aus `exercises.yml`).
